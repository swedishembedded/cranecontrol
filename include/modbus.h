...
int modbus_server_do_request(struct modbus_server *self, const uint8_t *rx, unsigned rx_size, uint8_t *tx, unsigned tx_max_size);

struct modbus_client *modbus_client_new();
void modbus_client_delete(struct modbus_client *self);
int modbus_client_read_u16(const struct modbus_client *self, uint8_t addr, uint16_t reg, uint16_t *val);
int modbus_client_write_u16(const struct modbus_client *self, uint8_t addr, uint16_t reg, uint16_t val);
