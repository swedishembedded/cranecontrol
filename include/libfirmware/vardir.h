...
#define VARDIR_LOOKUP_ENTRY(x)                                                                     \
	{                                                                                          \
		(x), sizeof(x) / sizeof(x[0])                                                      \
	}
static const struct vardir_lookup_entry _vardir_lookup_table[] = { VARDIR_LOOKUP_ENTRY(
	_vardir_lookup_on_off) };
...
#define VARDIR_ENTRY_VALUE(type, obj, member)                                                      \
	{                                                                                          \
		.base = (obj), .offset = offsetof(type, member)                                    \
	}
#define VARDIR_ENTRY_VALUE_CONST_PTR(type, obj, member)                                            \
	{                                                                                          \
		.base = (obj), .offset = offsetof(type, member)                                    \
	}
#define VARDIR_ENTRY_VALUE_SIMPLE(ptr)                                                             \
	{                                                                                          \
		.base = (ptr), .offset = 0                                                         \
	}
...
struct vardir_device_ops {
	int (*add_static)(vardir_device_t, uint32_t id, const char *name, vardir_value_type_t type,
			  const void *init_value);
	int (*add_dynamic)(vardir_device_t, uint32_t id, const char *name, vardir_value_type_t type,
			   struct vardir_entry_ops **ops);
	int (*get)(vardir_device_t, uint32_t id, const char *name, vardir_value_type_t get_as,
		   void *value, size_t value_size);
	int (*set)(vardir_device_t, uint32_t id, const char *name, vardir_value_type_t set_from,
		   const void *value);
};
...
int vardir_add_field(struct vardir *self, uint32_t id, const char *name, uint8_t type,
		     uint32_t default_value);
int vardir_add_entry(struct vardir *self, uint32_t id, const char *name, uint8_t type,
		     struct vardir_entry_ops **ops);
...
#if 0
#define vardir_add_i8(self, id, sub, name, value)                                                  \
	vardir_add_int(self, id, sub, name, VAR_INT8 | VAR_CONSTANT, value)
#define vardir_add_u8(self, id, sub, name, value)                                                  \
	vardir_add_int(self, id, sub, name, VAR_UINT8 | VAR_CONSTANT, value)
#define vardir_add_i16(self, id, sub, name, value)                                                 \
	vardir_add_int(self, id, sub, name, VAR_INT16 | VAR_CONSTANT, value)
#define vardir_add_u16(self, id, sub, name, value)                                                 \
	vardir_add_int(self, id, sub, name, VAR_UINT16 | VAR_CONSTANT, value)
#define vardir_add_i32(self, id, sub, name, value)                                                 \
	vardir_add_int(self, id, sub, name, VAR_INT32 | VAR_CONSTANT, value)
#define vardir_add_u32(self, id, sub, name, value)                                                 \
	vardir_add_int(self, id, sub, name, VAR_UINT32 | VAR_CONSTANT, value)
#endif
