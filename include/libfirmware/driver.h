...
#define DECLARE_DEVICE_CLASS(CLASS)                                                                \
	...
	void CLASS##_device_init(struct CLASS##_device *self, void *fdt, int fdt_node,             \
				 const struct CLASS##_device_ops *ops);                            \
...
#define DEFINE_DEVICE_CLASS(CLASS)                                                                 \
	...
	{                                                                                          \
		...
		{                                                                                  \
			if (dev->fdt == fdt && dev->fdt_node == node)                              \
				return &dev->ops;                                                  \
		}                                                                                  \
		return NULL;                                                                       \
	}                                                                                          \
...
#define DEVICE_DRIVER(_name, _compatible, _probe, _remove)                                         \
	void __attribute__((__constructor__, used)) _name##_ko()                                   \
	{                                                                                          \
		static struct device_driver __attribute__((used, aligned(4)))                      \
		_driver_##_name = { .name = #_name,                                                \
				    .compatible = _compatible,                                     \
				    .probe = _probe,                                               \
				    .remove = _remove };                                           \
		register_device_driver(&_driver_##_name);                                          \
	}                                                                                          \
	void system_use_##_name()                                                                  \
	{                                                                                          \
		static struct device_driver __attribute__((used, aligned(4)))                      \
		_driver_##_name = { .name = #_name,                                                \
				    .compatible = _compatible,                                     \
...
#define DEVICE_REF(name, type, ref)                                                                \
	do {                                                                                       \
