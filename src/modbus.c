...
int modbus_server_do_request(struct modbus_server *self, const uint8_t *rx,
                             unsigned rx_size, uint8_t *tx, unsigned tx_max_size) {
...
static void _modbus_server_task(void *ptr) {
	...
	while(1) {
		if(self->serial) {
			int res =
			    serial_read(self->serial, self->rx_data, sizeof(self->rx_data), 100000);
			if(res > 0) {
				if((res = modbus_server_do_request(self, self->rx_data, (unsigned)res,
				                                   self->tx_data, sizeof(self->tx_data))) >
				   0) {
...
int modbus_client_read_u16(const struct modbus_client *self, uint8_t addr,
...
int modbus_client_read_slave_id(const struct modbus_client *self, uint8_t addr,
                                uint8_t *id) {
	unsigned txi = 0;
	uint8_t tx[10];
	tx[txi++] = addr;
...
int modbus_client_write_u16(const struct modbus_client *self, uint8_t addr,
                            uint16_t reg, uint16_t val) {
	uint16_t txi = 0;
	uint8_t tx[10];
	tx[txi++] = addr;
