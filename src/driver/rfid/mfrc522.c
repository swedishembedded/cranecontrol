...
mfrc522_init(
	uint8_t chipSelectPin, ///< Arduino pin connected to MFRC522's SPI slave select input (Pin 24, NSS, active low)
...
)
{
	...
	SPI.transfer(
		reg &
		0x7E); // MSB == 0 is for writing. LSB is not used in address. Datasheet section 8.1.2.3.
	SPI.transfer(value);
...
)
{
	...
	SPI.transfer(
		0x80 |
		(reg &
		 0x7E)); // MSB == 1 is for reading. LSB is not used in address. Datasheet section 8.1.2.3.
...
)
{
	...
	while (index < count) {
		if (index == 0 && rxAlign) { // Only update bit positions rxAlign..7 in values[0]
			...
			values[index] = SPI.transfer(
				address); // Read value and tell that we want to read the same address again.
		}
		index++;
	}
...
)
{
	...
	return PCD_CommunicateWithPICC(PCD_Transceive, waitIRq, sendData, sendLen, backData,
...
)
{
	...
	PCD_ClearRegisterBitMask(
		CollReg, 0x80); // ValuesAfterColl=1 => Bits received after collision are cleared.
	validBits =
...
)
{
	...
	// Step 2: Transfer the data
	result = PCD_MIFARE_Transceive((byte *)&data, 4,
...
)
{
	...
	for (char blockOffset = no_of_blocks - 1; blockOffset >= 0; blockOffset--) {
		...
		if (group != 3 &&
		    (g[group] == 1 || g[group] == 6)) { // Not a sector trailer, a value block
