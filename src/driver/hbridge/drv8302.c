...
static int _drv8302_read(memory_device_t dev, size_t offs, void *data, size_t size)
{
	...
	if (size != 1)
		return -EINVAL;
	switch (offs) {
	case DRV8302_OFS_FAULT:
		*(uint8_t *)data = (uint8_t)gpio_read(self->gpio, PIN_FAULT);
		break;
	case DRV8302_OFS_OCW:
		*(uint8_t *)data = (uint8_t)gpio_read(self->gpio, PIN_OCW);
		break;
	case DRV8302_OFS_GAIN:
		*(uint8_t *)data = (uint8_t)gpio_read(self->gpio, PIN_GAIN);
		break;
	}
	return 1;
}
...
static struct memory_device_ops _memory_ops = { .read = _drv8302_read, .write = _drv8302_write };
