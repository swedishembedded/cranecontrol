...
#define enc28j60_writereadbyte(self, ch) (serial_putc(self->serial, ch), serial_getc(self->serial))
...
static void _enc28j60_set_bank(struct enc28j60 *self, uint8_t address)
{
	...
	if ((address & BANK_MASK) != self->bank) {
		// set the bank
		_enc28j60_write_reg(self, ENC28J60_BIT_FIELD_CLR, ECON1,
				    (ECON1_BSEL1 | ECON1_BSEL0));
		_enc28j60_write_reg(self, ENC28J60_BIT_FIELD_SET, ECON1,
				    (address & BANK_MASK) >> 5);
...
void enc28j60_init(struct enc28j60 *self, serial_dev_t serial, pio_dev_t port, gpio_pin_t cs_pin)
{
	...
	_enc28j60_write_reg(self, ENC28J60_BIT_FIELD_SET, MACON3,
