...
int _adns3080_write(struct adns3080 *self, uint8_t data)
{
	uint8_t tx[1] = { data };
	uint8_t rx[1] = { 0 };
	int ret = spi_transfer(self->spi, self->gpio, self->cs_pin, tx, rx, 1,
			       ADNS3080_TRANSFER_TIMEOUT);
	return ret;
}
...
int _adns3080_frame_capture(struct adns3080 *self)
{
	...
	for (size_t c = 0; c < ADNS3080_IMAGE_WIDTH * ADNS3080_IMAGE_HEIGHT; c++) {
		...
		sync = true;
		if (c != 0 && c % ADNS3080_IMAGE_WIDTH == 0)
			printk("\n");
		ch = (uint8_t) ~(ch << 2);
		(void)_letters;
...
static const struct pointer_device_ops _pointer_ops = { .read = _adns3080_pointer_read };
...
*/
	...
	uint8_t reg[5] = { ADNS3080_PRODUCT_ID, ADNS3080_REVISION_ID, ADNS3080_INVERSE_PRODUCT_ID,
			   ADNS3080_SROM_ID, ADNS3080_CONFIGURATION_BITS };
