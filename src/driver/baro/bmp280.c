...
static int _bmp280_read_pressure(baro_device_t dev, float *pressure){
    // start measurement
    uint32_t delay = ((T_INIT_MAX + T_MEASURE_PER_OSRS_MAX * (((1 <<
BMP280_TEMPERATURE_OSR) >> 1) + ((1 << BMP280_PRESSURE_OSR) >> 1)) +
(BMP280_PRESSURE_OSR ? T_SETUP_PRESSURE_MAX : 0) + 15) / 16);
    // set oversampling + power mode (forced), and start sampling
    if(i2c_write_reg(self->i2c, self->address, BMP280_CTRL_MEAS_REG, BMP280_MODE) <
0) return -EIO; thread_sleep_ms(delay);

    uint8_t data[BMP280_DATA_FRAME_SIZE];

    // read data from sensor
    i2c_read_buf(self->i2c, self->address, BMP280_PRESSURE_MSB_REG, data,
BMP280_DATA_FRAME_SIZE); int32_t up = (int32_t)((((uint32_t)(data[0])) << 12) |
(((uint32_t)(data[1])) << 4) | ((uint32_t)data[2] >> 4)); int32_t ut =
...
int _bmp280_probe(void *fdt, int fdt_node) {
	...
	baro_device_init(&self->dev, fdt, fdt_node, &_baro_ops);
	self->i2c = i2c;
	...
	/*
	    ...
	    baro->up_delay = ((T_INIT_MAX + T_MEASURE_PER_OSRS_MAX * (((1 <<
	   BMP280_TEMPERATURE_OSR) >> 1) + ((1 << BMP280_PRESSURE_OSR) >> 1)) +
	   (BMP280_PRESSURE_OSR ? T_SETUP_PRESSURE_MAX : 0) + 15) / 16) * 1000;
...
static int32_t bmp280_compensate_T(int32_t adc_T)
{
    ...
    var1 = ((((adc_T >> 3) - ((int32_t)self->cal_data.dig_T1 << 1))) *
((int32_t)self->cal_data.dig_T2)) >> 11; var2  = (((((adc_T >> 4) -
((int32_t)self->cal_data.dig_T1)) * ((adc_T >> 4) -
((int32_t)self->cal_data.dig_T1))) >> 12) * ((int32_t)self->cal_data.dig_T3)) >> 14;
...
static uint32_t bmp280_compensate_P(int32_t adc_P){
    ...
    var2 = var2 + ((var1*(int64_t)self->cal_data.dig_P5) << 17);
    var2 = var2 + (((int64_t)self->cal_data.dig_P4) << 35);
    var1 = ((var1 * var1 * (int64_t)self->cal_data.dig_P3) >> 8) + ((var1 *
(int64_t)self->cal_data.dig_P2) << 12); var1 = (((((int64_t)1) << 47) + var1)) *
((int64_t)self->cal_data.dig_P1) >> 33; if (var1 == 0) return 0; p = 1048576 - adc_P;
    p = (((p << 31) - var2) * 3125) / var1;
    var1 = (((int64_t)self->cal_data.dig_P9) * (p >> 13) * (p >> 13)) >> 25;
    var2 = (((int64_t)self->cal_data.dig_P8) * p) >> 19;
    p = ((p + var1 + var2) >> 8) + (((int64_t)self->cal_data.dig_P7) << 4);
