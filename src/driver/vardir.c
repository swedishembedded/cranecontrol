...
int _vardir_add_static(vardir_device_t dev, uint32_t id, const char *name, vardir_value_type_t type,
...
int _vardir_add_dynamic(vardir_device_t dev, uint32_t id, const char *name,
			vardir_value_type_t type, struct vardir_entry_ops **ops)
{
	struct vardir_driver *self = container_of(dev, struct vardir_driver, dev.ops);
	thread_mutex_lock(&self->lock);
...
int _vardir_get(vardir_device_t dev, uint32_t id, const char *name, vardir_value_type_t type,
		void *value, size_t size)
{
	...
	if (name) {
		if ((e = vardir_find_entry_by_name(&self->dir, name)))
			id = e->id;
		else {
			thread_mutex_unlock(&self->lock);
			return -ENOENT;
...
int _vardir_set(vardir_device_t dev, uint32_t id, const char *name, vardir_value_type_t type,
		const void *value)
{
	struct vardir_driver *self = container_of(dev, struct vardir_driver, dev.ops);
	struct vardir_entry *e = NULL;
	if (name)
		e = vardir_find_entry_by_name(&self->dir, name);
	else
		e = vardir_find_entry_by_id(&self->dir, id);
	if (!e)
		return -ENOENT;
