...
#define CHECK(x)                                                                                   \
	do {                                                                                       \
		if ((x) < 0) {                                                                     \
			printk(PRINT_ERROR "vl6180: i2c communication error!\n");                  \
			return -EIO;                                                               \
		}                                                                                  \
	} while (0)
...
static const struct analog_device_ops _vl6180_analog_ops = { .read = _vl6180_analog_read,
...
static int _vl6180_probe(void *fdt, int fdt_node)
{
	...
	uint8_t new_addr =
		(uint8_t)fdt_get_int_or_default(fdt, fdt_node, "reg", VL6180_DEFAULT_ADDRESS);
	...
	case VL6180_MODEL_ID: {
		printk(PRINT_DEFAULT
		       "vl6180: model rev: %d.%d, module rev: %d.%d, date: %d, time %d\n",
		       rev_major, rev_minor, mod_major, mod_minor, date, time);
	} break;
	default: {
