...
int wiinunchuck_getjoyX(struct wiinunchuck *self)
{
	...
	if (joyX <= WIINUNCHUCK_CENTERJOYX)
		joyX = (joyX - WIINUNCHUCK_INMINJOYX) * (127 - 0) /
			       (WIINUNCHUCK_CENTERJOYX - WIINUNCHUCK_INMINJOYX) +
		       0;
	else
		joyX = (joyX - WIINUNCHUCK_CENTERJOYX) * (255 - 127) /
			       (WIINUNCHUCK_INMAXJOYX - WIINUNCHUCK_CENTERJOYX) +
		       127 - 1;
	return joyX - 127;
#else
...
int wiinunchuck_getjoyY(struct wiinunchuck *self)
{
	...
	if (joyY <= WIINUNCHUCK_CENTERJOYY)
		joyY = (joyY - WIINUNCHUCK_INMINJOYY) * (127 - 0) /
			       (WIINUNCHUCK_CENTERJOYY - WIINUNCHUCK_INMINJOYY) +
		       0;
	else
		joyY = (joyY - WIINUNCHUCK_CENTERJOYY) * (255 - 127) /
			       (WIINUNCHUCK_INMAXJOYY - WIINUNCHUCK_CENTERJOYY) +
		       127 - 1;
	return joyY - 127;
#else
...
void wiinunchuck_getpitchroll(struct wiinunchuck *self, double ax, double ay, double az,
			      double *pitch, double *roll)
{
	(void)(self);
	double magnitude = sqrt(((uint32_t)ax * (uint32_t)ax) + ((uint32_t)ay * (uint32_t)ay) +
				((uint32_t)az * (uint32_t)az));
	ax = ax / magnitude;
	ay = ay / magnitude;
	az = az / magnitude;
 ...
 */
static int wiinunchuck_avaragefilter(struct wiinunchuck *self, int input,
				     int *wiinunchuck_avarageangle)
{
	(void)(self);
	uint8_t i = 0;
	long sum = 0;
