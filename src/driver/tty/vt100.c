	...
	case EV_CHAR: {
		if (isdigit(arg)) { // start of an argument
			...
			case 'B': { // cursor down (cursor stops at bottom margin)
				unsigned n = (term->narg > 0) ? term->args[0] : 1;
				term->cursor_y += n;
				if (term->cursor_y > term->screen_height)
					term->cursor_y = term->screen_height;
				term->state = _st_idle;
				break;
			}
			case 'C': { // cursor right (cursor stops at right margin)
				...
				if (term->cursor_x > term->screen_width)
					term->cursor_x = term->screen_width;
				term->state = _st_idle;
				break;
			}
			case 'D': { // cursor left
			...
			case 'J': { // clear screen from cursor up or down
				...
				if (term->narg == 0 || (term->narg == 1 && term->args[0] == 0)) {
					...
					_vt100_clearLines(term, term->cursor_y,
							  term->screen_height);
			...
			case 'K': { // clear line from cursor right/left
				...
				if (term->narg == 0 || (term->narg == 1 && term->args[0] == 0)) {
					...
					// including cursor
					for (unsigned c = term->cursor_x; c < term->screen_width;
					     c++)
						_vt100_putc(term, ' ');
					...
					term->cursor_x = 0;
					for (unsigned c = term->cursor_x; c < cx; c++)
						_vt100_putc(term, ' ');
...
serial_dev_t vt100_get_serial_interface(struct vt100 *self)
{
	...
	_if = (struct serial_if){ .put = _vt100_serial_putc,
