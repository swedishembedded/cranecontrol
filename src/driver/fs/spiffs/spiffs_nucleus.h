...
#define SPIFFS_ERASE_COUNT_PADDR(fs, bix)                                                          \
	(SPIFFS_BLOCK_TO_PADDR(fs, bix) +                                                          \
	 SPIFFS_OBJ_LOOKUP_PAGES(fs) * SPIFFS_CFG_LOG_PAGE_SZ(fs) - sizeof(spiffs_obj_id))
...
#define SPIFFS_OBJ_IX_ENTRY(fs, spix)                                                              \
	((spix) < SPIFFS_OBJ_HDR_IX_LEN(fs) ?                                                      \
		 (spix) :                                                                          \
		 (((spix)-SPIFFS_OBJ_HDR_IX_LEN(fs)) % SPIFFS_OBJ_IX_LEN(fs)))
// object index span index number for given data span index or entry
#define SPIFFS_OBJ_IX_ENTRY_SPAN_IX(fs, spix)                                                      \
	((spix) < SPIFFS_OBJ_HDR_IX_LEN(fs) ?                                                      \
		 0 :                                                                               \
		 (1 + ((spix)-SPIFFS_OBJ_HDR_IX_LEN(fs)) / SPIFFS_OBJ_IX_LEN(fs)))
...
#define SPIFFS_CHECK_MOUNT(fs) ((fs)->block_count > 0)
...
#define SPIFFS_VALIDATE_OBJIX(ph, objid, spix)                                                     \
	if (((ph).flags & SPIFFS_PH_FLAG_USED) != 0)                                               \
		return SPIFFS_ERR_IS_FREE;                                                         \
	if (((ph).flags & SPIFFS_PH_FLAG_DELET) == 0)                                              \
		return SPIFFS_ERR_DELETED;                                                         \
	if (((ph).flags & SPIFFS_PH_FLAG_FINAL) != 0)                                              \
		return SPIFFS_ERR_NOT_FINALIZED;                                                   \
	if (((ph).flags & SPIFFS_PH_FLAG_INDEX) != 0)                                              \
		return SPIFFS_ERR_NOT_INDEX;                                                       \
	if (((objid)&SPIFFS_OBJ_ID_IX_FLAG) == 0)                                                  \
		return SPIFFS_ERR_NOT_INDEX;                                                       \
	if ((ph).span_ix != (spix))                                                                \
		return SPIFFS_ERR_INDEX_SPAN_MISMATCH;
...
#define SPIFFS_VALIDATE_DATA(ph, objid, spix)                                                      \
	...
	if (((ph).flags & SPIFFS_PH_FLAG_FINAL) != 0)                                              \
		return SPIFFS_ERR_NOT_FINALIZED;                                                   \
	if (((ph).flags & SPIFFS_PH_FLAG_INDEX) == 0)                                              \
		return SPIFFS_ERR_IS_INDEX;                                                        \
	if ((objid)&SPIFFS_OBJ_ID_IX_FLAG)                                                         \
		return SPIFFS_ERR_IS_INDEX;                                                        \
	if ((ph).span_ix != (spix))                                                                \
		return SPIFFS_ERR_DATA_SPAN_MISMATCH;
...
#define SPIFFS_CACHE_PAGE_SIZE(fs) (sizeof(spiffs_cache_page) + SPIFFS_CFG_LOG_PAGE_SZ(fs))

#define spiffs_get_cache(fs) ((spiffs_cache *)((fs)->cache))
...
typedef struct __attribute((packed)) {
	...
	// alignment
	u8_t _align[4 - (sizeof(spiffs_page_header) & 3) == 0 ? 4 :
...
#if SPIFFS_CACHE
		     u8_t op, spiffs_file fh,
#endif
		     u32_t addr, u32_t len, u8_t *dst);

s32_t spiffs_phys_wr(spiffs *fs,
#if SPIFFS_CACHE
		     u8_t op, spiffs_file fh,
#endif
		     u32_t addr, u32_t len, u8_t *src);
...
s32_t spiffs_object_open_by_id(spiffs *fs, spiffs_obj_id obj_id, spiffs_fd *f, spiffs_flags flags,
			       spiffs_mode mode);

s32_t spiffs_object_open_by_page(spiffs *fs, spiffs_page_ix pix, spiffs_fd *f, spiffs_flags flags,
				 spiffs_mode mode);
...
s32_t spiffs_gc_find_candidate(spiffs *fs, spiffs_block_ix **block_candidate, int *candidate_count);
