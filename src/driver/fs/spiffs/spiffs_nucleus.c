				    ...
				    spiffs_span_ix spix)
{
	...
	res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_READ, fd->file_nbr,
...
s32_t spiffs_obj_lu_find_entry_visitor(spiffs *fs, spiffs_block_ix starting_block,
				       int starting_lu_entry, u8_t flags, spiffs_obj_id obj_id,
				       spiffs_visitor_f v, u32_t user_data, void *user_p,
				       spiffs_block_ix *block_ix, int *lu_entry)
{
	...
	while (res == SPIFFS_OK && entry_count > 0) {
		...
		while (res == SPIFFS_OK && obj_lookup_page < SPIFFS_OBJ_LOOKUP_PAGES(fs)) {
			...
			// check each entry
			while (res == SPIFFS_OK &&
			       cur_entry - entry_offset <
				       entries_per_page && // for non-last obj lookup pages
			       cur_entry < SPIFFS_OBJ_LOOKUP_MAX_ENTRIES(
						   fs)) // for last obj lookup page
			{
				if ((flags & SPIFFS_VIS_CHECK_ID) == 0 ||
					...
					if (v) {
						res = v(fs,
							...
							cur_block, cur_entry, user_data, user_p);
...
static s32_t spiffs_obj_lu_scan_v(spiffs *fs, spiffs_obj_id obj_id, spiffs_block_ix bix,
				  int ix_entry, u32_t user_data, void *user_p)
{
...
s32_t spiffs_obj_lu_scan(spiffs *fs)
{
	...
	res = spiffs_obj_lu_find_entry_visitor(fs, 0, 0, 0, 0, spiffs_obj_lu_scan_v, 0, 0, &bix,
					       &entry);
			      ...
			      spiffs_block_ix *block_ix, int *lu_entry)
{
	...
	res = spiffs_obj_lu_find_id(fs, starting_block, starting_lu_entry, SPIFFS_OBJ_ID_FREE,
				    block_ix, lu_entry);
	if (res == SPIFFS_OK) {
...
// Iterate over object lookup pages in each block until a given object id entry is found
s32_t spiffs_obj_lu_find_id(spiffs *fs, spiffs_block_ix starting_block, int starting_lu_entry,
			    spiffs_obj_id obj_id, spiffs_block_ix *block_ix, int *lu_entry)
{
	s32_t res = spiffs_obj_lu_find_entry_visitor(fs, starting_block, starting_lu_entry,
						     SPIFFS_VIS_CHECK_ID, obj_id, 0, 0, 0, block_ix,
						     lu_entry);
	if (res == SPIFFS_VIS_END) {
...
static s32_t spiffs_obj_lu_find_id_and_span_v(spiffs *fs, spiffs_obj_id obj_id, spiffs_block_ix bix,
					      int ix_entry, u32_t user_data, void *user_p)
{
	s32_t res;
	spiffs_page_header ph;
	...
	res = _spiffs_rd(fs, 0, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ,
			 SPIFFS_PAGE_TO_PADDR(fs, pix), sizeof(spiffs_page_header), (u8_t *)&ph);
	SPIFFS_CHECK_RES(res);
	if (ph.obj_id == obj_id && ph.span_ix == (spiffs_span_ix)user_data &&
	    (ph.flags & (SPIFFS_PH_FLAG_FINAL | SPIFFS_PH_FLAG_DELET | SPIFFS_PH_FLAG_USED)) ==
		    SPIFFS_PH_FLAG_DELET &&
	    !((obj_id & SPIFFS_OBJ_ID_IX_FLAG) && (ph.flags & SPIFFS_PH_FLAG_IXDELE) == 0 &&
	      ph.span_ix == 0) &&
				     ...
				     spiffs_page_ix exclusion_pix, spiffs_page_ix *pix)
{
	...
	res = spiffs_obj_lu_find_entry_visitor(fs, fs->cursor_block_ix, fs->cursor_obj_lu_entry,
					       SPIFFS_VIS_CHECK_ID, obj_id,
					       spiffs_obj_lu_find_id_and_span_v, (u32_t)spix,
					       exclusion_pix ? &exclusion_pix : 0, &bix, &entry);
...
s32_t spiffs_page_allocate_data(spiffs *fs, spiffs_obj_id obj_id, spiffs_page_header *ph,
				u8_t *data, u32_t len, u32_t page_offs, u8_t finalize,
				spiffs_page_ix *pix)
{
	s32_t res = SPIFFS_OK;
	spiffs_block_ix bix;
	...
	// find free entry
	res = spiffs_obj_lu_find_free(fs, fs->free_cursor_block_ix, fs->free_cursor_obj_lu_entry,
				      &bix, &entry);
	SPIFFS_CHECK_RES(res);

	// occupy page in object lookup
	res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_UPDT, 0,
			 SPIFFS_BLOCK_TO_PADDR(fs, bix) + entry * sizeof(spiffs_obj_id),
	...
	res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT, 0,
			 SPIFFS_OBJ_LOOKUP_ENTRY_TO_PADDR(fs, bix, entry),
...
s32_t spiffs_page_move(spiffs *fs, spiffs_file fh, u8_t *page_data, spiffs_obj_id obj_id,
		       spiffs_page_header *page_hdr, spiffs_page_ix src_pix,
		       spiffs_page_ix *dst_pix)
{
	...
	if (page_data) {
		...
		res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT, 0,
				 SPIFFS_PAGE_TO_PADDR(fs, free_pix), SPIFFS_CFG_LOG_PAGE_SZ(fs),
				 page_data);
	} else {
		// copy page data
		res = spiffs_phys_cpy(fs, fh, SPIFFS_PAGE_TO_PADDR(fs, free_pix),
			   ...
			   spiffs_obj_type type, spiffs_page_ix *objix_hdr_pix)
{
	...
	oix_hdr.type = type;
	oix_hdr.size =
		SPIFFS_UNDEFINED_LEN; // keep ones so we can update later without wasting this page
	...
	SPIFFS_CHECK_RES(res);
	spiffs_cb_object_event(fs, 0, SPIFFS_EV_IX_NEW, obj_id, 0,
...
s32_t spiffs_object_update_index_hdr(spiffs *fs, spiffs_fd *fd, spiffs_obj_id obj_id,
				     spiffs_page_ix objix_hdr_pix, u8_t *new_objix_hdr_data,
				     u8_t name[SPIFFS_OBJ_NAME_LEN], u32_t size,
				     spiffs_page_ix *new_pix)
{
	...
	res = spiffs_page_move(fs, fd == 0 ? 0 : fd->file_nbr, (u8_t *)objix_hdr, obj_id, 0,
			       objix_hdr_pix, &new_objix_hdr_pix);
	...
	if (res == SPIFFS_OK) {
		...
		spiffs_cb_object_event(fs, fd, SPIFFS_EV_IX_UPD, obj_id, objix_hdr->p_hdr.span_ix,
				       new_objix_hdr_pix, objix_hdr->size);
		if (fd)
			fd->objix_hdr_pix =
				new_objix_hdr_pix; // if this is not in the registered cluster
	}

	return res;
}

void spiffs_cb_object_event(spiffs *fs, spiffs_fd *fd, int ev, spiffs_obj_id obj_id,
			    spiffs_span_ix spix, spiffs_page_ix new_pix, u32_t new_size)
{
	...
	for (i = 0; i < fs->fd_count; i++) {
		...
		if (spix == 0) {
			if (ev == SPIFFS_EV_IX_NEW || ev == SPIFFS_EV_IX_UPD) {
				SPIFFS_DBG(
					"       callback: setting fd %i:%04x objix_hdr_pix to %04x, size:%i\n",
		...
		if (cur_fd->cursor_objix_spix == spix) {
			if (ev == SPIFFS_EV_IX_NEW || ev == SPIFFS_EV_IX_UPD) {
				SPIFFS_DBG(
					"       callback: setting fd %i:%04x span:%04x objix_pix to %04x\n",
...
// Open object by id
s32_t spiffs_object_open_by_id(spiffs *fs, spiffs_obj_id obj_id, spiffs_fd *fd, spiffs_flags flags,
			       spiffs_mode mode)
{
	s32_t res = SPIFFS_OK;
	spiffs_page_ix pix;
...
s32_t spiffs_object_open_by_page(spiffs *fs, spiffs_page_ix pix, spiffs_fd *fd, spiffs_flags flags,
				 spiffs_mode mode)
{
	s32_t res = SPIFFS_OK;
	spiffs_page_object_ix_header oix_hdr;
...
s32_t spiffs_object_append(spiffs_fd *fd, u32_t offset, u8_t *data, u32_t len)
{
	...
	while (res == SPIFFS_OK && written < len) {
		...
		if (cur_objix_spix != prev_objix_spix) {
			...
			if (written > 0) {
				...
				SPIFFS_DBG("append: %04x store objix %04x:%04x, written %i\n",
					   fd->obj_id, cur_objix_pix, prev_objix_spix, written);
				if (prev_objix_spix == 0) {
					...
					if (offset == 0) {
						...
						res = spiffs_page_index_check(fs, fd, cur_objix_pix,
									      0);
						SPIFFS_CHECK_RES(res);
						res = _spiffs_wr(
							fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_UPDT,
							fd->file_nbr,
							SPIFFS_PAGE_TO_PADDR(fs, cur_objix_pix),
							SPIFFS_CFG_LOG_PAGE_SZ(fs), fs->work);
						SPIFFS_CHECK_RES(res);
					} else {
						// was a nonempty object, update to new page
						res = spiffs_object_update_index_hdr(
							fs, fd, fd->obj_id, fd->objix_hdr_pix,
							fs->work, 0, offset + written,
							&new_objix_hdr_page);
						SPIFFS_CHECK_RES(res);
						SPIFFS_DBG(
							"append: %04x store new objix_hdr, %04x:%04x, written %i\n",
					...
					res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_UPDT,
							 ...
							 SPIFFS_CFG_LOG_PAGE_SZ(fs), fs->work);
					SPIFFS_CHECK_RES(res);
					spiffs_cb_object_event(fs, fd, SPIFFS_EV_IX_UPD, fd->obj_id,
					...
					SPIFFS_CHECK_RES(res);
					SPIFFS_DBG(
						"append: %04x store new size I %i in objix_hdr, %04x:%04x, written %i\n",
						fd->obj_id, offset + written, new_objix_hdr_page, 0,
						written);
				}
			...
			if (cur_objix_spix == 0) {
				...
				spiffs_span_ix len_objix_spix = SPIFFS_OBJ_IX_ENTRY_SPAN_IX(
					fs, (fd->size - 1) / SPIFFS_DATA_PAGE_SIZE(fs));
				...
				if (written > 0 || cur_objix_spix > len_objix_spix) {
					...
					p_hdr.flags = 0xff & ~(SPIFFS_PH_FLAG_FINAL |
							       SPIFFS_PH_FLAG_INDEX);
					res = spiffs_page_allocate_data(
						fs, fd->obj_id | SPIFFS_OBJ_ID_IX_FLAG, &p_hdr, 0,
						0, 0, 1, &cur_objix_pix);
					SPIFFS_CHECK_RES(res);
					spiffs_cb_object_event(fs, fd, SPIFFS_EV_IX_NEW, fd->obj_id,
					...
					if (fd->cursor_objix_spix == cur_objix_spix) {
						...
						res = spiffs_obj_lu_find_id_and_span(
							fs, fd->obj_id | SPIFFS_OBJ_ID_IX_FLAG,
		...
		if (page_offs == 0) {
			...
			res = spiffs_page_allocate_data(fs, fd->obj_id & ~SPIFFS_OBJ_ID_IX_FLAG,
							&p_hdr, &data[written], to_write, page_offs,
							1, &data_page);
			SPIFFS_DBG(
				"append: %04x store new data page, %04x:%04x offset:%i, len %i, written %i\n",
			...
			if (cur_objix_spix == 0) {
				...
				data_page = ((spiffs_page_ix *)((void *)objix +
			...
			SPIFFS_DBG(
				"append: %04x store to existing data page, %04x:%04x offset:%i, len %i, written %i\n",
				fd->obj_id, data_page, data_spix, page_offs, to_write, written);
		}
		...
		if (cur_objix_spix == 0) {
			...
			((spiffs_page_ix *)((void *)objix + sizeof(spiffs_page_object_ix)))
	...
	if (cur_objix_spix != 0) {
		...
		res2 = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_UPDT, fd->file_nbr,
				  ...
				  SPIFFS_CFG_LOG_PAGE_SZ(fs), fs->work);
		SPIFFS_CHECK_RES(res2);
		spiffs_cb_object_event(fs, fd, SPIFFS_EV_IX_UPD, fd->obj_id, objix->p_hdr.span_ix,
				       cur_objix_pix, 0);

		// update size in object header index page
		res2 = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, 0, 0,
		...
		if (offset == 0) {
			...
			objix_hdr->size = offset + written;
			SPIFFS_DBG(
				"append: %04x store fresh objix_hdr page, %04x:%04x, written %i\n",
				fd->obj_id, cur_objix_pix, cur_objix_spix, written);
			...
			res2 = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix,
							      fs->work, 0, offset + written,
							      &new_objix_hdr_page);
			SPIFFS_DBG(
				"append: %04x store modified objix_hdr page, %04x:%04x, written %i\n",
				fd->obj_id, new_objix_hdr_page, 0, written);
			SPIFFS_CHECK_RES(res2);
		}
	}
...
s32_t spiffs_object_modify(spiffs_fd *fd, u32_t offset, u8_t *data, u32_t len)
{
	...
	while (res == SPIFFS_OK && written < len) {
			...
			if (written > 0) {
				...
				if (prev_objix_spix == 0) {
					...
					res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id,
									     ...
									     &new_objix_hdr_pix);
					SPIFFS_DBG(
						"modify: store modified objix_hdr page, %04x:%04x, written %i\n",
					...
					res = spiffs_page_move(fs, fd->file_nbr, (u8_t *)objix,
							       fd->obj_id, 0, cur_objix_pix,
							       &new_objix_pix);
					SPIFFS_DBG(
						"modify: store previous modified objix page, %04x:%04x, written %i\n",
		...
		if (page_offs == 0 && to_write == SPIFFS_DATA_PAGE_SIZE(fs)) {
			...
			spiffs_page_allocate_data(fs, fd->obj_id & ~SPIFFS_OBJ_ID_IX_FLAG, &p_hdr,
						  &data[written], to_write, page_offs, 1,
						  &data_pix);
			SPIFFS_DBG(
				"modify: store new data page, %04x:%04x offset:%i, len %i, written %i\n",
			...
			SPIFFS_DBG(
				"modify: store to existing data page, src:%04x, dst:%04x:%04x offset:%i, len %i, written %i\n",
	...
	if (cur_objix_spix != 0) {
		...
		res2 = spiffs_page_move(fs, fd->file_nbr, (u8_t *)objix, fd->obj_id, 0,
...
static s32_t spiffs_object_find_object_index_header_by_name_v(spiffs *fs, spiffs_obj_id obj_id,
							      ...
							      u32_t user_data, void *user_p)
{
	...
	if (objix_hdr.p_hdr.span_ix == 0 &&
	    (objix_hdr.p_hdr.flags &
	     (SPIFFS_PH_FLAG_DELET | SPIFFS_PH_FLAG_FINAL | SPIFFS_PH_FLAG_IXDELE)) ==
...
s32_t spiffs_object_find_object_index_header_by_name(spiffs *fs, u8_t name[SPIFFS_OBJ_NAME_LEN],
						     spiffs_page_ix *pix)
{
	s32_t res;
	spiffs_block_ix bix;
	...
	res = spiffs_obj_lu_find_entry_visitor(fs, fs->cursor_block_ix, fs->cursor_obj_lu_entry, 0,
					       0, spiffs_object_find_object_index_header_by_name_v,
					       0, name, &bix, &entry);
...
s32_t spiffs_object_truncate(spiffs_fd *fd, u32_t new_size, u8_t remove)
{
	...
	if (remove && new_size == 0) {
		...
		res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_UPDT, fd->file_nbr,
				 SPIFFS_PAGE_TO_PADDR(fs, fd->objix_hdr_pix) +
					 offsetof(spiffs_page_header, flags),
	...
	while (cur_size > new_size) {
		...
		if (prev_objix_spix != cur_objix_spix) {
			if (prev_objix_spix != (spiffs_span_ix)-1) {
				...
				SPIFFS_CHECK_RES(res);
				spiffs_cb_object_event(fs, fd, SPIFFS_EV_IX_DEL, fd->obj_id,
			...
			if (cur_objix_spix == 0) {
				...
				res = spiffs_obj_lu_find_id_and_span(
					fs, fd->obj_id | SPIFFS_OBJ_ID_IX_FLAG, cur_objix_spix, 0,
					&objix_pix);
				SPIFFS_CHECK_RES(res);
			}
		...
		if (cur_size - SPIFFS_DATA_PAGE_SIZE(fs) >= new_size) {
			...
			fd->offset = cur_size;
			SPIFFS_DBG(
				"truncate: delete data page %04x for data spix:%04x, cur_size:%i\n",
				data_pix, data_spix, cur_size);
		} else {
			// delete last page, partially
			spiffs_page_header p_hdr;
			spiffs_page_ix new_data_pix;
			u32_t bytes_to_remove =
				SPIFFS_DATA_PAGE_SIZE(fs) - (new_size % SPIFFS_DATA_PAGE_SIZE(fs));
			SPIFFS_DBG(
				"truncate: delete %i bytes from data page %04x for data spix:%04x, cur_size:%i\n",
			...
			p_hdr.flags &= ~SPIFFS_PH_FLAG_FINAL;
			res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT, fd->file_nbr,
					 SPIFFS_PAGE_TO_PADDR(fs, new_data_pix) +
						 offsetof(spiffs_page_header, flags),
					 sizeof(u8_t), (u8_t *)&p_hdr.flags);
			if (res != SPIFFS_OK)
				break;
	...
	if (cur_objix_spix == 0) {
		...
		if (cur_size == 0) {
			if (remove) {
				...
				res = spiffs_page_delete(fs, objix_pix);
				SPIFFS_CHECK_RES(res);
				spiffs_cb_object_event(fs, fd, SPIFFS_EV_IX_DEL, fd->obj_id, 0,
						       objix_pix, 0);
			} else {
				...
				res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, objix_pix,
								     fs->work, 0,
								     SPIFFS_UNDEFINED_LEN,
								     &new_objix_hdr_pix);
				SPIFFS_CHECK_RES(res);
		...
		res = spiffs_page_move(fs, fd->file_nbr, (u8_t *)objix_hdr, fd->obj_id, 0,
...
s32_t spiffs_object_read(spiffs_fd *fd, u32_t offset, u32_t len, u8_t *dst)
{
	...
	while (cur_offset < offset + len) {
		...
		len_to_read = MIN(len_to_read, SPIFFS_DATA_PAGE_SIZE(fs) -
						      ...
						      u32_t user_data, void *user_p)
{
	if (id != SPIFFS_OBJ_ID_FREE && id != SPIFFS_OBJ_ID_DELETED &&
		...
		res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0,
				 SPIFFS_OBJ_LOOKUP_ENTRY_TO_PADDR(fs, bix, ix_entry),
				 sizeof(spiffs_page_header), (u8_t *)&ph);
		if (res == SPIFFS_OK && ph.span_ix == 0 &&
		    ((ph.flags & (SPIFFS_PH_FLAG_INDEX | SPIFFS_PH_FLAG_FINAL |
	...
	while (res == SPIFFS_OK && free_obj_id == SPIFFS_OBJ_ID_FREE) {
		if (state.max_obj_id - state.min_obj_id <= SPIFFS_CFG_LOG_PAGE_SZ(fs) * 8) {
			...
			res = spiffs_obj_lu_find_entry_visitor(
				fs, 0, 0, 0, 0, spiffs_obj_lu_find_free_obj_id_bitmap_v,
				state.min_obj_id, 0, 0, 0);
			if (res == SPIFFS_VIS_END)
				res = SPIFFS_OK;
			SPIFFS_CHECK_RES(res);
			...
			if (state.compaction != 0) {
				...
				SPIFFS_DBG(
					"free_obj_id: COMP select index:%i min_count:%i min:%04x max:%04x compact:%i\n",
					min_i, min_count, state.min_obj_id, state.max_obj_id,
					state.compaction);
				...
				if (min_count == 0) {
					...
					*obj_id = min_i * state.compaction + state.min_obj_id;
					...
					SPIFFS_DBG(
						"free_obj_id: COMP SEL chunk:%04x min:%04x -> %04x\n",
						state.compaction, state.min_obj_id,
			...
			memset(fs->work, 0, SPIFFS_CFG_LOG_PAGE_SZ(fs));
			res = spiffs_obj_lu_find_entry_visitor(
				fs, 0, 0, 0, 0, spiffs_obj_lu_find_free_obj_id_compact_v, 0, &state,
				0, 0);
			if (res == SPIFFS_VIS_END)
				res = SPIFFS_OK;
			SPIFFS_CHECK_RES(res);
