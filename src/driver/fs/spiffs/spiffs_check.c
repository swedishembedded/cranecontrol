...
// destroys fs->lu_work
static s32_t spiffs_object_get_data_page_index_reference(spiffs *fs, spiffs_obj_id obj_id,
							 ...
							 spiffs_page_ix *objix_pix)
{
	...
	res = spiffs_obj_lu_find_id_and_span(fs, obj_id | SPIFFS_OBJ_ID_IX_FLAG, objix_spix, 0,
					     objix_pix);
	...
	if (objix_spix == 0) {
		...
		addr += sizeof(spiffs_page_object_ix) +
			SPIFFS_OBJ_IX_ENTRY(fs, data_spix) * sizeof(spiffs_page_ix);
	}

	res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0, addr,
			 sizeof(spiffs_page_ix), (u8_t *)pix);
				  ...
				  spiffs_page_ix new_data_pix, spiffs_page_ix objix_pix)
{
	...
	res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0,
			 SPIFFS_PAGE_TO_PADDR(fs, objix_pix), SPIFFS_CFG_LOG_PAGE_SZ(fs),
			 fs->lu_work);
	SPIFFS_CHECK_RES(res);
	...
	if ((objix_p_hdr->flags &
	     (SPIFFS_PH_FLAG_USED | SPIFFS_PH_FLAG_IXDELE | SPIFFS_PH_FLAG_INDEX |
	...
	// rewrite in mem
	if (objix_spix == 0) {
		((spiffs_page_ix *)((void *)fs->lu_work +
...
static s32_t spiffs_lookup_check_validate(spiffs *fs, spiffs_obj_id lu_obj_id,
					  spiffs_page_header *p_hdr, spiffs_page_ix cur_pix,
					  spiffs_block_ix cur_block, int cur_entry, int *reload_lu)
{
	u8_t delete_page = 0;
	s32_t res = SPIFFS_OK;
	spiffs_page_ix objix_pix;
		...
		if (p_hdr->flags & SPIFFS_PH_FLAG_INDEX) {
			...
			if (res == SPIFFS_ERR_NOT_FOUND) {
				...
				if (ref_pix == cur_pix) {
					...
					SPIFFS_CHECK_DBG(
						"LU: FIXUP: data page not found elsewhere, rewriting %04x to new page %04x\n",
						cur_pix, new_pix);
					SPIFFS_CHECK_RES(res);
					*reload_lu = 1;
					SPIFFS_CHECK_DBG(
						"LU: FIXUP: %04x rewritten to %04x, affected objix_pix %04x\n",
			...
			if (res == SPIFFS_ERR_NOT_FOUND) {
				...
				res = spiffs_obj_lu_find_id_and_span_by_phdr(
					fs, p_hdr->obj_id | SPIFFS_OBJ_ID_IX_FLAG, 0, 0, 0);
	...
	if (lu_obj_id != SPIFFS_OBJ_ID_FREE && lu_obj_id != SPIFFS_OBJ_ID_DELETED) {
		...
		if ((p_hdr->obj_id | SPIFFS_OBJ_ID_IX_FLAG) !=
			...
			if ((p_hdr->flags & SPIFFS_PH_FLAG_DELET) == 0 ||
				...
				if (p_hdr->flags & SPIFFS_PH_FLAG_INDEX) {
					...
					res = spiffs_object_get_data_page_index_reference(
						fs, p_hdr->obj_id, p_hdr->span_ix, &ref_pix,
						&objix_pix);
					if (res == SPIFFS_ERR_NOT_FOUND) {
						// no object with this id, so remove page safely
						res = SPIFFS_OK;
						...
						if (ref_pix == cur_pix) {
							spiffs_page_ix new_pix;
							res = spiffs_rewrite_page(fs, cur_pix,
										  p_hdr, &new_pix);
							SPIFFS_CHECK_RES(res);
							res = spiffs_rewrite_index(
								fs, p_hdr->obj_id, p_hdr->span_ix,
								new_pix, objix_pix);
							if (res <= _SPIFFS_ERR_CHECK_FIRST &&
							    res > _SPIFFS_ERR_CHECK_LAST) {
								// index bad also, cannot mend this file
					...
					res = spiffs_obj_lu_find_id_and_span(
						fs, lu_obj_id | SPIFFS_OBJ_ID_IX_FLAG,
						p_hdr->span_ix, 0, &objix_pix_lu);
					if (res == SPIFFS_ERR_NOT_FOUND) {
						res = SPIFFS_OK;
						objix_pix_lu = 0;
					}
					...
					res = spiffs_obj_lu_find_id_and_span(
						fs, p_hdr->obj_id | SPIFFS_OBJ_ID_IX_FLAG,
						p_hdr->span_ix, 0, &objix_pix_ph);
					if (res == SPIFFS_ERR_NOT_FOUND) {
						res = SPIFFS_OK;
						objix_pix_ph = 0;
					}
					...
					if (objix_pix_ph == 0 || objix_pix_lu == 0) {
						...
						res = spiffs_obj_lu_find_id_and_span(
							fs, lu_obj_id & ~SPIFFS_OBJ_ID_IX_FLAG, 0,
							0, &data_pix_lu);
						if (res == SPIFFS_ERR_NOT_FOUND) {
							res = SPIFFS_OK;
							objix_pix_lu = 0;
						...
						spiffs_page_ix new_pix;
						if ((objix_pix_lu && data_pix_lu && data_pix_ph &&
						     objix_pix_ph == 0) ||
						    (objix_pix_lu == 0 && data_pix_ph &&
						     objix_pix_ph == 0)) {
							//   got a data page for page header obj id
							//   rewrite as obj_id_ph
							new_ph.obj_id = p_hdr->obj_id |
									SPIFFS_OBJ_ID_IX_FLAG;
							res = spiffs_rewrite_page(
								fs, cur_pix, &new_ph, &new_pix);
							SPIFFS_CHECK_DBG(
							...
							SPIFFS_CHECK_RES(res);
							*reload_lu = 1;
						} else if ((objix_pix_ph && data_pix_ph &&
							    data_pix_lu && objix_pix_lu == 0) ||
							   (objix_pix_ph == 0 && data_pix_lu &&
							    objix_pix_lu == 0)) {
		...
		} else if (((lu_obj_id & SPIFFS_OBJ_ID_IX_FLAG) &&
			...
			res = spiffs_obj_lu_find_id_and_span(fs, lu_obj_id & ~SPIFFS_OBJ_ID_IX_FLAG,
			...
			res = spiffs_obj_lu_find_id_and_span(fs, lu_obj_id | SPIFFS_OBJ_ID_IX_FLAG,
							     p_hdr->span_ix, cur_pix, &objix_pix);
			if (res == SPIFFS_ERR_NOT_FOUND) {
				res = SPIFFS_OK;
				objix_pix = 0;
			}
			...
			if (data_pix && objix_pix) {
				...
				if (data_pix && objix_pix == 0) {
					...
					if (fs->check_cb_f)
						fs->check_cb_f(SPIFFS_CHECK_LOOKUP,
							       SPIFFS_CHECK_FIX_INDEX, lu_obj_id,
							       p_hdr->span_ix);
					spiffs_page_header new_ph;
					spiffs_page_ix new_pix;
					new_ph.flags = 0xff & ~(SPIFFS_PH_FLAG_USED |
			...
			*reload_lu = 1;
			...
			if (res == SPIFFS_ERR_NOT_FOUND) {
				...
				if (ref_pix != cur_pix) {
					...
					res = _spiffs_wr(
						fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT, 0,
						SPIFFS_PAGE_TO_PADDR(fs, cur_pix) +
							offsetof(spiffs_page_header, flags),
						sizeof(u8_t), (u8_t *)&flags);
				}
	...
	if (delete_page) {
		...
		if (fs->check_cb_f)
			fs->check_cb_f(SPIFFS_CHECK_LOOKUP, SPIFFS_CHECK_DELETE_PAGE, cur_pix, 0);
				   ...
				   int cur_entry, u32_t user_data, void *user_p)
{
	...
	if (fs->check_cb_f)
		fs->check_cb_f(SPIFFS_CHECK_LOOKUP, SPIFFS_CHECK_PROGRESS,
	...
	res = spiffs_lookup_check_validate(fs, obj_id, &p_hdr, cur_pix, cur_block, cur_entry,
					   &reload_lu);
	SPIFFS_CHECK_RES(res);
...
s32_t spiffs_lookup_consistency_check(spiffs *fs, u8_t check_all_objects)
{
	...
	if (fs->check_cb_f)
		fs->check_cb_f(SPIFFS_CHECK_LOOKUP, SPIFFS_CHECK_PROGRESS, 0, 0);
	...
	if (res != SPIFFS_OK) {
		if (fs->check_cb_f)
			fs->check_cb_f(SPIFFS_CHECK_LOOKUP, SPIFFS_CHECK_ERROR, res, 0);
	}

	if (fs->check_cb_f)
		fs->check_cb_f(SPIFFS_CHECK_LOOKUP, SPIFFS_CHECK_PROGRESS, 256, 0);
...
static s32_t spiffs_page_consistency_check_i(spiffs *fs)
{
	...
	while (pix_offset < SPIFFS_PAGES_PER_BLOCK(fs) * fs->block_count) {
		...
		while (!restart && cur_block < fs->block_count) {
			if (fs->check_cb_f)
				fs->check_cb_f(SPIFFS_CHECK_PAGE, SPIFFS_CHECK_PROGRESS,
					       (pix_offset * 256) / (SPIFFS_PAGES_PER_BLOCK(fs) *
								     fs->block_count) +
			...
			spiffs_page_ix cur_pix = SPIFFS_OBJ_LOOKUP_PAGES(fs) +
			...
			while (!restart && cur_pix < SPIFFS_PAGES_PER_BLOCK(fs) * (cur_block + 1)) {
				...
				u8_t within_range = (cur_pix >= pix_offset &&
				...
				if (within_range && (p_hdr.flags & SPIFFS_PH_FLAG_DELET) &&
				...
				if ((p_hdr.flags & SPIFFS_PH_FLAG_DELET) &&
				    ...
				    (p_hdr.flags & (SPIFFS_PH_FLAG_INDEX | SPIFFS_PH_FLAG_USED)) ==
					    0) {
					...
					for (i = 0; !restart && i < entries; i++) {
						...
						if ((rpix != (spiffs_page_ix)-1 &&
							...
							// bad reference
							SPIFFS_CHECK_DBG(
								"PA: pix %04x bad pix / LU referenced from page %04x\n",
								rpix, cur_pix);
							...
							if (data_pix == 0) {
								...
								res = spiffs_page_allocate_data(
									fs, new_ph.obj_id, &new_ph,
									0, 0, 0, 1, &data_pix);
								SPIFFS_CHECK_RES(res);
								SPIFFS_CHECK_DBG(
									"PA: FIXUP: found no existing data page, created new @ %04x\n",
									data_pix);
							}
							// remap index
							SPIFFS_CHECK_DBG(
								"PA: FIXUP: rewriting index pix %04x\n",
								cur_pix);
							res = spiffs_rewrite_index(
								fs,
								objix_p_hdr->obj_id |
							...
							if (rp_hdr.obj_id !=
								...
								const u32_t rpix_byte_ix =
									(rpix - pix_offset) /
									(8 / bits);
								const u8_t rpix_bit_ix =
									(rpix & ((8 / bits) - 1)) *
									bits;
								if (fs->work[rpix_byte_ix] &
								    (1 << (rpix_bit_ix + 1))) {
									SPIFFS_CHECK_DBG(
										"PA: pix %04x multiple referenced from page %04x\n",
										rpix, cur_pix);
			...
			for (byte_ix = 0; !restart && byte_ix < SPIFFS_CFG_LOG_PAGE_SZ(fs);
				...
				for (bit_ix = 0; !restart && bit_ix < 8 / bits; bit_ix++) {
					...
					if (bitmask == 0x1) {
						// 001
						SPIFFS_CHECK_DBG(
							"PA: pix %04x USED, UNREFERENCED, not index\n",
							cur_pix);
						...
						res = spiffs_object_get_data_page_index_reference(
							fs, p_hdr.obj_id, p_hdr.span_ix, &rpix,
							&objix_pix);
						if (res == SPIFFS_OK) {
							if (((rpix == (spiffs_page_ix)-1 ||
							      rpix > SPIFFS_MAX_PAGES(fs)) ||
								...
								SPIFFS_CHECK_DBG(
									"PA: corresponding ref is bad: %04x, rewrite to this %04x\n",
									rpix, cur_pix);
							} else {
								...
								if (((p_hdr.obj_id &
									...
									SPIFFS_CHECK_DBG(
										"PA: corresponding ref is good but different: %04x, delete this %04x\n",
										rpix, cur_pix);
									delete_page = 1;
								} else {
									...
									if (rpix != cur_pix) {
										SPIFFS_CHECK_DBG(
											"PA: corresponding ref is weird: %04x %s%s%s%s, rewrite this %04x\n",
											rpix,
											(rp_hdr.flags &
											 SPIFFS_PH_FLAG_INDEX) ?
												"" :
							...
							SPIFFS_CHECK_DBG(
								"PA: corresponding ref not found, delete %04x\n",
								cur_pix);
							delete_page = 1;
							res = SPIFFS_OK;
						}
						...
						if (rewrite_ix_to_this) {
							...
							SPIFFS_CHECK_DBG(
								"PA: FIXUP: rewrite index id %04x data spix %04x to point to this pix: %04x\n",
								p_hdr.obj_id, p_hdr.span_ix,
								cur_pix);
							res = spiffs_rewrite_index(fs, p_hdr.obj_id,
										   p_hdr.span_ix,
					...
					if (bitmask == 0x2) {
						// 010
						SPIFFS_CHECK_DBG(
							"PA: pix %04x FREE, REFERENCED, not index\n",
							cur_pix);
					...
					if (bitmask == 0x4) {
						// 100
						SPIFFS_CHECK_DBG(
							"PA: pix %04x FREE, unreferenced, INDEX\n",
							cur_pix);
					...
					if (bitmask == 0x6) {
						// 110
						SPIFFS_CHECK_DBG(
							"PA: pix %04x FREE, REFERENCED, INDEX\n",
							cur_pix);
					...
					if (bitmask == 0x7) {
						// 111
						SPIFFS_CHECK_DBG(
							"PA: pix %04x USED, REFERENCED, INDEX\n",
							cur_pix);
...
{
	if (fs->check_cb_f)
		fs->check_cb_f(SPIFFS_CHECK_PAGE, SPIFFS_CHECK_PROGRESS, 0, 0);
	s32_t res = spiffs_page_consistency_check_i(fs);
	if (res != SPIFFS_OK) {
		if (fs->check_cb_f)
			fs->check_cb_f(SPIFFS_CHECK_PAGE, SPIFFS_CHECK_ERROR, res, 0);
	}
	if (fs->check_cb_f)
		fs->check_cb_f(SPIFFS_CHECK_PAGE, SPIFFS_CHECK_PROGRESS, 256, 0);
	return res;
}
...
s32_t spiffs_object_index_consistency_check_v(spiffs *fs, spiffs_obj_id obj_id,
					      ...
					      u32_t user_data, void *user_p)
{
	...
	if (fs->check_cb_f)
		fs->check_cb_f(SPIFFS_CHECK_INDEX, SPIFFS_CHECK_PROGRESS,
			       (cur_block * 256) / fs->block_count, 0);
	...
	if (obj_id != SPIFFS_OBJ_ID_FREE && obj_id != SPIFFS_OBJ_ID_DELETED &&
		...
		if (p_hdr.span_ix == 0 &&
		    (p_hdr.flags & (SPIFFS_PH_FLAG_INDEX | SPIFFS_PH_FLAG_FINAL |
			...
			if (fs->check_cb_f)
				fs->check_cb_f(SPIFFS_CHECK_INDEX, SPIFFS_CHECK_DELETE_PAGE,
					       cur_pix, obj_id);
			res = spiffs_page_delete(fs, cur_pix);
			SPIFFS_CHECK_RES(res);
			return res_c;
		...
		if ((p_hdr.flags & (SPIFFS_PH_FLAG_INDEX | SPIFFS_PH_FLAG_FINAL |
				    SPIFFS_PH_FLAG_DELET | SPIFFS_PH_FLAG_IXDELE)) ==
		    (SPIFFS_PH_FLAG_DELET | SPIFFS_PH_FLAG_IXDELE)) {
			return res_c;
		}
		...
		if (p_hdr.span_ix == 0) {
			...
			if (r == -1) {
				...
				spiffs_page_ix objix_hdr_pix;
				res = spiffs_obj_lu_find_id_and_span(
					fs, obj_id | SPIFFS_OBJ_ID_IX_FLAG, 0, 0, &objix_hdr_pix);
			...
			if (delete) {
				SPIFFS_CHECK_DBG(
					...
					cur_pix, obj_id, p_hdr.span_ix);
				if (fs->check_cb_f)
					fs->check_cb_f(SPIFFS_CHECK_INDEX,
						       SPIFFS_CHECK_DELETE_ORPHANED_INDEX, cur_pix,
						       obj_id);
				res = spiffs_page_delete(fs, cur_pix);
				SPIFFS_CHECK_RES(res);
			}
...
s32_t spiffs_object_index_consistency_check(spiffs *fs)
{
	...
	if (fs->check_cb_f)
		fs->check_cb_f(SPIFFS_CHECK_INDEX, SPIFFS_CHECK_PROGRESS, 0, 0);
	res = spiffs_obj_lu_find_entry_visitor(
		fs, 0, 0, 0, 0, spiffs_object_index_consistency_check_v, 0, &obj_id_log_ix, 0, 0);
	if (res == SPIFFS_VIS_END) {
		res = SPIFFS_OK;
	}
	if (res != SPIFFS_OK) {
		if (fs->check_cb_f)
			fs->check_cb_f(SPIFFS_CHECK_INDEX, SPIFFS_CHECK_ERROR, res, 0);
	}
	if (fs->check_cb_f)
		fs->check_cb_f(SPIFFS_CHECK_INDEX, SPIFFS_CHECK_PROGRESS, 256, 0);
	return res;
}
