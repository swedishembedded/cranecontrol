...
{
	return sizeof(spiffs_cache) +
	       num_pages * (sizeof(spiffs_cache_page) + SPIFFS_CFG_LOG_PAGE_SZ(fs));
}
#endif
#endif

s32_t SPIFFS_mount(spiffs *fs, spiffs_config *config, u8_t *work, u8_t *fd_space,
		   u32_t fd_space_size, void *cache, u32_t cache_size,
		...
		if (len < SPIFFS_CFG_LOG_PAGE_SZ(fs)) {
			...
			if (fd->cache_page) {
				...
				if (offset < fd->cache_page->offset || // writing before cache
				    offset > fd->cache_page->offset +
						     fd->cache_page->size || // writing after cache
				    offset + len > fd->cache_page->offset +
							   SPIFFS_CFG_LOG_PAGE_SZ(
								   fs)) // writing beyond cache page
				{
					// boundary violation, write back cache first and allocate new
					SPIFFS_CACHE_DBG(
						"CACHE_WR_DUMP: dumping cache page %i for fd %i:&04x, boundary viol, offs:%i size:%i\n",
			...
			if (alloc_cpage) {
				...
				if (fd->cache_page) {
					...
					SPIFFS_CACHE_DBG(
						"CACHE_WR_ALLO: allocating cache page %i for fd %i:%04x\n",
						fd->cache_page->ix, fd->file_nbr, fd->obj_id);
				}
			}
			...
			if (fd->cache_page) {
				...
				SPIFFS_CACHE_DBG(
					"CACHE_WR_WRITE: storing to cache page %i for fd %i:%04x, offs %i:%i len %i\n",
					fd->cache_page->ix, fd->file_nbr, fd->obj_id, offset,
					offset_in_cpage, len);
			...
			if (fd->cache_page) {
				...
				SPIFFS_CACHE_DBG(
					"CACHE_WR_DUMP: dumping cache page %i for fd %i:%04x, big write, offs:%i size:%i\n",
					fd->cache_page->ix, fd->file_nbr, fd->obj_id,
					fd->cache_page->offset, fd->cache_page->size);
...
s32_t SPIFFS_lseek(spiffs *fs, spiffs_file fh, s32_t offs, int whence)
{
	...
	if (fd->cursor_objix_spix != objix_spix) {
		spiffs_page_ix pix;
		res = spiffs_obj_lu_find_id_and_span(fs, fd->obj_id | SPIFFS_OBJ_ID_IX_FLAG,
						     objix_spix, 0, &pix);
		SPIFFS_API_CHECK_RES_UNLOCK(fs, res);
...
static s32_t spiffs_fflush_cache(spiffs *fs, spiffs_file fh)
{
	...
	if ((fd->flags & SPIFFS_DIRECT) == 0) {
		...
		if (fd->cache_page) {
			SPIFFS_CACHE_DBG(
				"CACHE_WR_DUMP: dumping cache page %i for fd %i:%04x, flush, offs:%i size:%i\n",
				fd->cache_page->ix, fd->file_nbr, fd->obj_id,
				fd->cache_page->offset, fd->cache_page->size);
...
struct spiffs_dirent *SPIFFS_readdir(spiffs_DIR *d, struct spiffs_dirent *e)
{
	...
	res = spiffs_obj_lu_find_entry_visitor(d->fs, d->block, d->entry, SPIFFS_VIS_NO_WRAP, 0,
