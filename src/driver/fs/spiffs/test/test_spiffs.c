...
void hexdump(u32_t addr, u32_t len)
{
	...
	for (j = 0; j < 32; j++) {
		if (a - 32 + j < addr)
			...
			printf("%c", (area[a - 32 + j] < 32 || area[a - 32 + j] >= 0x7f) ?
					     '.' :
					     area[a - 32 + j]);
		}
	}
		...
		if (obj_id & SPIFFS_OBJ_ID_IX_FLAG) {
			...
			if (ph->span_ix == 0) {
				...
				spiffs_page_object_ix_header *oix_hdr =
					(spiffs_page_object_ix_header *)&area[addr];
				printf("'%s'  %i bytes  type:%02x", oix_hdr->name, oix_hdr->size,
				       oix_hdr->type);
			}
		} else {
			// data page
...
void dump_erase_counts(spiffs *fs)
{
	...
	for (bix = 0; bix < fs->block_count; bix++) {
		...
		_spiffs_rd(fs, 0, 0, SPIFFS_ERASE_COUNT_PADDR(fs, bix), sizeof(spiffs_obj_id),
			   (u8_t *)&erase_mark);
		if (erases[bix] == 0) {
			printf("            |");
		} else {
...
void dump_flash_access_stats()
{
	printf("  RD: %10i reads  %10i bytes %10i avg bytes/read\n", reads, bytes_rd,
	       reads == 0 ? 0 : (bytes_rd / reads));
	printf("  WR: %10i writes %10i bytes %10i avg bytes/write\n", writes, bytes_wr,
	       writes == 0 ? 0 : (bytes_wr / writes));
}

static u32_t old_perc = 999;
static void spiffs_check_cb_f(spiffs_check_type type, spiffs_check_report report, u32_t arg1,
			      u32_t arg2)
{
...
u32_t tfile_get_size(tfile_size s)
{
	...
	case MEDIUM:
		return SPIFFS_DATA_PAGE_SIZE(FS) *
		...
		for (i = 0; i < max_concurrent_files; i++) {
			...
			if (tf->state == 0 && cur_config_ix < cfg_count) {
				...
				if (tf->cfg.tsize == EMPTY) {
					...
					CHECK_RES(res);
					int pfd =
						open(make_test_fname(name),
						     O_TRUNC | O_CREAT | O_RDWR, S_IRUSR | S_IWUSR);
					close(pfd);
					int extra_flags =
						tf->cfg.ttype == APPENDED ? SPIFFS_APPEND : 0;
					spiffs_file fd =
						SPIFFS_open(FS, name, extra_flags | SPIFFS_RDWR, 0);
					CHECK(fd > 0);
					tf->fd = fd;
				} else {
					int extra_flags =
					...
					spiffs_file fd =
						SPIFFS_open(FS, name,
							    extra_flags | SPIFFS_TRUNC |
								    SPIFFS_CREAT | SPIFFS_RDWR,
							    0);
					CHECK(fd > 0);
					extra_flags = tf->cfg.ttype == APPENDED ? O_APPEND : 0;
					int pfd = open(make_test_fname(name),
						       extra_flags | O_TRUNC | O_CREAT | O_RDWR,
						       S_IRUSR | S_IWUSR);
					tf->fd = fd;
				...
				case APPENDED: {
					...
					CHECK_RES(res);
					int pfd =
						open(make_test_fname(tf->name), O_APPEND | O_RDWR);
					write(pfd, buf, size);
					close(pfd);
					free(buf);
				...
				case MODIFIED: {
					...
					CHECK_RES(res);
					int size = stat.size / tf->cfg.tlife +
						   SPIFFS_DATA_PAGE_SIZE(FS) / 3;
				...
				case REWRITTEN: {
					...
					if (dbg)
						printf("   rewriting %s\n", tf->name);
					spiffs_file fd = SPIFFS_open(
						FS, tf->name,
						SPIFFS_TRUNC | SPIFFS_CREAT | SPIFFS_RDWR, 0);
					CHECK(fd > 0);
					int pfd =
						open(make_test_fname(tf->name),
						     O_TRUNC | O_CREAT | O_RDWR, S_IRUSR | S_IWUSR);
					tf->fd = fd;
					int size = tfile_get_size(tf->cfg.tsize);
					u8_t *buf = malloc(size);
