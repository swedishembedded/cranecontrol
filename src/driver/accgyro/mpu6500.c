...
enum { MPU_CLK_INTERNAL = 0, MPU_CLK_PLL };

enum { MPU_ACC_FSR_2G = 0, MPU_ACC_FSR_4G, MPU_ACC_FSR_8G, MPU_ACC_FSR_16G };
...
static int _mpu6500_read_reg(struct mpu6500 *self, uint8_t reg, uint8_t *data)
{
	...
	if (spi_transfer(self->spi, self->gpio, self->cs_pin, tx, rx, 2, MPU6500_REQUEST_TIMEOUT) <
	    0) {
		return -EIO;
	}
	*data = rx[1];
...
static int _mpu6500_write_reg(struct mpu6500 *self, uint8_t reg, uint8_t data)
{
	if (self->i2c) {
		...
		return i2c_transfer(self->i2c, MPU_DEFAULT_I2C_ADDR, buf, 2, NULL, 0,
				    MPU6500_TIMEOUT);
	} else if (self->spi) {
		uint8_t tx[2] = { reg, data };
		uint8_t rx[2] = { 0, 0 };
		// try to read out the id of the chip
		if (spi_transfer(self->spi, self->gpio, self->cs_pin, tx, rx, 2,
				 MPU6500_REQUEST_TIMEOUT) < 0) {
			return -EIO;
		}
		return 1;
	}
...
{
	if (self->i2c) {
		return i2c_transfer(self->i2c, MPU_DEFAULT_I2C_ADDR, &reg, 1, data, len,
				    MPU6500_TIMEOUT);
	} else if (self->spi) {
		uint8_t tx[0xf + 1] = { reg | 0x80, 0 };
		uint8_t rx[0xf + 1] = { 0 };
		// try to read out the id of the chip
		if (spi_transfer(self->spi, self->gpio, self->cs_pin, tx, rx, 1 + (len & 0xf),
				 MPU6500_REQUEST_TIMEOUT) < 0) {
			return -EIO;
		}
...
static const struct imu_device_ops _imu_ops = { .read = _mpu6500_read };
...
int _mpu6500_probe(void *fdt, int fdt_node)
{
	...
	if (sig == MPU6500_ID)
		dbg_printk("mpu6500: found mpu6500\n");
	else if (sig == MPU9250_ID)
		dbg_printk("mpu6500: found mpu9250\n");
	else if (sig == ICM20689_ID)
		dbg_printk("mpu6500: found icm20689\n");
	else
		dbg_printk(PRINT_ERROR "mpu6500: unknown device (%02x)\n", sig);
