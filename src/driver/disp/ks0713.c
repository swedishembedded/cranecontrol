...
/**
  ...
  * @retval None
  */
void ks0713_init(struct ks0713 *self,
		 void (*putn)(struct ks0713 *self, uint16_t *data, size_t size))
{
	...
	ks0713_send_command(self, KS0713_ADC_SELECT |
					  0x01); // Select SEG output direction reversed (ADC = 1)
	ks0713_send_command(self, KS0713_REVERSE_DISP |
					  0x00); // Select normal / reverse display (REV = 0)
	...
	ks0713_send_command(self, KS0713_SHL_SELECT |
					  0x08); // Select COM output direction normal (SHL = 0)
	ks0713_send_command(self, KS0713_POWER_CTRL |
					  0x07); // Control power circuit operation (VC,VR,VF on)
	ks0713_send_command(self,
			    KS0713_REG_RES_SEL | 0x04); // Select internal resistance ratio (0x05)
	ks0713_send_command(self,
			    KS0713_SET_REF_VOLTAGE); // Set reference voltage Mode (2-part cmd)
...
{
	self->contrast = val;
	if ((self->contrast + val) > 0xff)
		self->contrast = 0xff;
	else if (self->contrast + val < 0)
		self->contrast = 0;
...
{
	if (x > KS0713_WIDTH || y > KS0713_HEIGHT)
		return;
...
{
	if (((y * KS0713_CHAR_HEIGHT) + KS0713_CHAR_HEIGHT) > KS0713_HEIGHT)
		return;
	if (((x * KS0713_CHAR_WIDTH) + KS0713_CHAR_WIDTH) > KS0713_WIDTH)
		return;
...
tty_dev_t ks0713_get_tty_interface(struct ks0713 *self)
{
	...
	_if = (struct tty_device){ .put = _ks0713_put,
...
void ks0713_draw_line(struct ks0713 *self, int8_t x0, int8_t y0, int8_t x1, int8_t y1,
		      ks0713_pixel_op_t op)
{
...
void ks0713_draw_rect(struct ks0713 *self, int8_t x1, int8_t y1, int8_t x2, int8_t y2,
		      ks0713_pixel_op_t op)
{
	int8_t x, y;
