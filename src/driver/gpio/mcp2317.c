 ...
 * Copyright (c) Cleanflight contributors
 *
...
static void _mcp2317_write_reg_bits(struct mcp2317 *self, uint8_t reg, uint8_t bits, uint8_t values)
{
...
static int _mcp2317_write_pin(gpio_device_t dev, uint32_t pin, bool value)
{
	...
	if (pin < 8) {
		...
		_mcp2317_write_reg(self, MCP2317_REG_GPIOA,
				   (uint8_t)((_mcp2317_read_reg(self, MCP2317_REG_GPIOA) &
					      (uint32_t) ~(1 << pin)) |
					     ((uint32_t)value << pin)));
		thread_mutex_unlock(&self->lock);
	} else {
...
static struct gpio_device_ops _mcp2317_ops = { .write_pin = _mcp2317_write_pin,
...
static int _mcp2317_probe(void *fdt, int fdt_node)
{
	...
	if (console) {
		console_add_command(console, self, fdt_get_name(fdt, fdt_node, NULL),
				    "mcp2317 device control", "", _mcp2317_cmd);
	}
	...
	if (_mcp2317_read_reg(self, 0x00) != 0xff || _mcp2317_read_reg(self, 0x01) != 0xff) {
	...
	if (val && len > 0) {
		if ((len * 4) % 3 != 0) {
			...
			for (int c = 0; c < pin_count; c++) {
				...
				_mcp2317_write_reg_bits(self, reg_iodir, (uint8_t)(1 << idx),
							(uint8_t)((options & 1) << idx));
				_mcp2317_write_reg_bits(self, reg_gppu, (uint8_t)(1 << idx),
							(uint8_t)(((options >> 1) & 1) << idx));
				_mcp2317_write_reg_bits(self, reg_gpio, (uint8_t)(1 << idx),
