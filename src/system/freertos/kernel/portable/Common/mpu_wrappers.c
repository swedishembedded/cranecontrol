			   ...
			   TaskHandle_t *pxCreatedTask)
{
	...
	xReturn = xTaskCreate(pvTaskCode, pcName, usStackDepth, pvParameters, uxPriority,
			      pxCreatedTask);
	vPortResetPrivilege(xRunningPrivileged);
	return xReturn;
}
				   ...
				   StaticTask_t *const pxTaskBuffer)
{
	...
	xReturn = xTaskCreateStatic(pxTaskCode, pcName, ulStackDepth, pvParameters, uxPriority,
...
#if (configUSE_TRACE_FACILITY == 1)
UBaseType_t MPU_uxTaskGetSystemState(TaskStatus_t *pxTaskStatusArray, UBaseType_t uxArraySize,
			       ...
			       uint32_t *pulNotificationValue, TickType_t xTicksToWait)
{
	...
	xReturn = xTaskNotifyWait(ulBitsToClearOnEntry, ulBitsToClearOnExit, pulNotificationValue,
				  xTicksToWait);
	vPortResetPrivilege(xRunningPrivileged);
	return xReturn;
}
...
#if (configSUPPORT_DYNAMIC_ALLOCATION == 1)
QueueHandle_t MPU_xQueueGenericCreate(UBaseType_t uxQueueLength, UBaseType_t uxItemSize,
				      uint8_t ucQueueType)
{
					    ...
					    StaticQueue_t *pxStaticQueue, const uint8_t ucQueueType)
{
	...
	xReturn = xQueueGenericCreateStatic(uxQueueLength, uxItemSize, pucQueueStorage,
			       ...
			       TimerCallbackFunction_t pxCallbackFunction)
{
	...
	xReturn = xTimerCreate(pcTimerName, xTimerPeriodInTicks, uxAutoReload, pvTimerID,
			       pxCallbackFunction);
	vPortResetPrivilege(xRunningPrivileged);
				     ...
				     StaticTimer_t *pxTimerBuffer)
{
	...
	xReturn = xTimerCreateStatic(pcTimerName, xTimerPeriodInTicks, uxAutoReload, pvTimerID,
				    ...
				    TickType_t xTicksToWait)
{
	...
	xReturn = xEventGroupWaitBits(xEventGroup, uxBitsToWaitFor, xClearOnExit, xWaitForAllBits,
				      xTicksToWait);
	vPortResetPrivilege(xRunningPrivileged);
