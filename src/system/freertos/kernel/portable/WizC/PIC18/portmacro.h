...
#define portMINIMAL_STACK_SIZE                                                                     \
	((usCalcMinStackSize == 0) ? usPortCALCULATE_MINIMAL_STACK_SIZE() : usCalcMinStackSize)
...
#define portSAVE_CONTEXT(ucInterruptForced)                                                        \
	do {                                                                                       \
		...
		movff STATUS, POSTDEC2 movff WREG, POSTDEC2 movff BSR, POSTDEC2 movff PRODH,       \
			POSTDEC2 movff PRODL, POSTDEC2 movff FSR0H, POSTDEC2 movff FSR0L,          \
			POSTDEC2 movff FSR1H, POSTDEC2 movff FSR1L, POSTDEC2 movff TABLAT,         \
			POSTDEC2 if __ROMSIZE > 0x8000 movff TBLPTRU,                              \
			POSTDEC2 endif movff TBLPTRH, POSTDEC2 movff TBLPTRL,                      \
			POSTDEC2 if __ROMSIZE > 0x8000 movff PCLATU, POSTDEC2 endif movff PCLATH,  \
			POSTDEC2;                                                                  \
		;                                                                                  \
		Store the compiler - scratch - area as described above.;                           \
		movlw OVERHEADPAGE0 - LOCOPTSIZE + MAXLOCOPTSIZE clrf FSR0L, ACCESS clrf FSR0H,    \
			ACCESS _rtos_S1 : movff POSTINC0,                                          \
		...
		stack, too.;                                                                       \
		movf STKPTR, W, ACCESS bz _rtos_s3 _rtos_S2 : if __ROMSIZE > 0x8000 movff TOSU,    \
...
#define portRESTORE_CONTEXT()                                                                      \
	do {                                                                                       \
		...
		movlw OVERHEADPAGE0 - LOCOPTSIZE + MAXLOCOPTSIZE movwf FSR0L, ACCESS clrf FSR0H,   \
			ACCESS _rtos_R2 : decf FSR0L,                                              \
					  F,                                                       \
