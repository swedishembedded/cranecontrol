...
#define portSAVE_CONTEXT()                                                                         \
	{                                                                                          \
		_asm /* Push ACC first, as when restoring the context it must be restored					\
		last (it is used to set the IE register). */   \
			push ACC /* Store the IE register then disable interrupts. */              \
				push IE clr _EA push DPL push DPH push b push ar2 push ar3 push    \
					ar4 push ar5 push ar6 push ar7 push ar0 push ar1 push PSW  \
						_endasm;                                           \
		PSW = 0;                                                                           \
...
#define portRESTORE_CONTEXT()                                                                                  \
	{                                                                                                      \
		_asm pop _bp pop PSW pop ar1 pop ar0 pop ar7 pop ar6 pop ar5 pop ar4 pop ar3 pop               \
			ar2 pop b pop DPH pop                                                                  \
				DPL /* The next byte of the stack is the IE register.  Only the global						\
