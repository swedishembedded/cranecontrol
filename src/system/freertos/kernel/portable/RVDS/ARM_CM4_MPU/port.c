...
static const volatile uint8_t *const pcInterruptPriorityRegisters =
	(const uint8_t *)portNVIC_IP_REGISTERS_OFFSET_16;
...
BaseType_t xPortStartScheduler(void)
{
...
#if (configASSERT_DEFINED == 1)
	{
		...
		volatile uint8_t *const pucFirstUserPriorityRegister =
			(volatile uint8_t *)(portNVIC_IP_REGISTERS_OFFSET_16 +
...
# 0x10 /* Is the task using the FPU context?  If so, push high vfp registers. */
	...
	str r0,
		...
		stmdb sp !,
		{ r3 } mov r0, #configMAX_SYSCALL_INTERRUPT_PRIORITY msr basepri,
