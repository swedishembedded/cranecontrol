...
#if (configUSE_COMMON_INTERRUPT_ENTRY_POINT == 1)
	{
		for (ulNum = 0; ulNum < portNUM_VECTORS; ulNum++) {
			...
			if ((xInterruptDescriptorTable[ulNum].usISRLow == 0x00) &&
				...
				prvSetInterruptGate((uint8_t)ulNum, vPortCentralInterruptWrapper,
						    portIDT_FLAGS);
			}
		}
	}
...
static void prvSetupTimerInterrupt(void)
{
	...
	prvSetInterruptGate((uint8_t)portAPIC_LVT_ERROR_VECTOR, vPortAPICErrorHandlerWrapper,
			    portIDT_FLAGS);
	...
	prvSetInterruptGate((uint8_t)portAPIC_SPURIOUS_INT_VECTOR, vPortAPICSpuriousHandler,
			    portIDT_FLAGS);
...
BaseType_t xPortInstallInterruptHandler(ISR_Handler_t pxHandler, uint32_t ulVectorNumber)
{
	...
	if (xReturn != pdFAIL) {
		taskENTER_CRITICAL();
		{
			...
			prvSetInterruptGate((uint8_t)ulVectorNumber, (ISR_Handler_t)pxHandler,
					    portIDT_FLAGS);
		}
		taskEXIT_CRITICAL();
	}
