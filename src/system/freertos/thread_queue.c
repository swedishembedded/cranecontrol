...
{
	if (xQueueSend(self->handle, data, tout_ms) == pdTRUE)
		return 1;
	return -1;
}
...
{
	if (xQueueSendFromISR(self->handle, data, wake) == pdTRUE)
		return 1;
	return -1;
}
...
{
	if (xQueueReceive(self->handle, data, tout_ms) == pdTRUE)
		return 1;
	return -1;
}
...
{
	if (xQueueReceiveFromISR(self->handle, data, wake) == pdTRUE)
		return 1;
	return -1;
}
...
{
	if (xQueuePeek(self->handle, data, tout_ms) == pdTRUE)
		return 0;
	return -1;
}
...
{
	if (xQueueOverwrite(self->handle, data) == pdTRUE)
		return 0;
	return -1;
}
