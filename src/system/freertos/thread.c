...
{
	if (xTaskCreate(thread, name, (uint16_t)stack_words, ptr, priority, handle) == pdPASS)
		return 0;
	return -ENOMEM;
}
...
#define CONSOLE_MAX_PS_TASKS 12
	...
	UBaseType_t ret =
		uxTaskGetSystemState(status, sizeof(status) / sizeof(status[0]), &total_time);
	struct timeval tval;
	time_gettime(&tval);
	if (ret > 0) {
		...
		for (UBaseType_t c = 0; c < ret; c++) {
			...
			if (total_calculated && dtotal) {
				cpu_percent =
					(uint32_t)((uint64_t)run_time * 10000 / total_calculated);
			...
			printk("%5u%5u%8s%8u%10u%5u.%02u%5u.%02u\n", stat->xTaskNumber,
			       stat->uxBasePriority, stat->pcTaskName, stat->usStackHighWaterMark,
			       stat->ulRunTimeCounter / (portTICK_PERIOD_MS * 1000), cpu_whole,
			       cpu_frac, dcpu_whole, dcpu_frac);
		}
