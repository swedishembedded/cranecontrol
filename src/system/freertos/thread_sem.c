...
{
	if ((self->sem = xSemaphoreCreateBinary()) == NULL)
		return -1;
	return 0;
}
...
{
	if ((self->sem = xSemaphoreCreateCounting(max_count, initial_count)) == NULL)
		return -1;
	return 0;
}
...
{
	if (xSemaphoreTake(self->sem, portMAX_DELAY) == pdTRUE)
		return 0;
	return -1;
}
...
{
	if (xSemaphoreTake(self->sem, (timeout == THREAD_SLEEP_MAX_DELAY) ?
					      (portMAX_DELAY) :
					      (timeout / portTICK_PERIOD_MS)) == pdTRUE)
		return 0;
	return -1;
}
...
{
	if (xSemaphoreGive(self->sem) == pdTRUE)
		return 0;
	return -1;
}
