...
void work_init(struct work *self, void (*handler)(struct work *))
{
	...
	priv->timer = xTimerCreate("work", 1, false, self, _run_work);
...
int queue_work(struct work *self, uint32_t delay_ms)
{
	...
	if (xTimerIsTimerActive(priv->timer))
		return 0;

	xTimerChangePeriod(priv->timer, delay_ms / portTICK_PERIOD_MS,
			   WORK_SETUP_BLOCK_TIME / portTICK_PERIOD_MS);
...
int queue_work_from_isr(struct work *self, uint32_t delay_ms, int32_t *_wake)
{
	struct work_priv *priv = (struct work_priv *)self->priv;
	BaseType_t wake;
	if (xTimerChangePeriodFromISR(priv->timer, delay_ms / portTICK_PERIOD_MS, &wake) == pdFAIL)
		return -1;
	if (xTimerStartFromISR(priv->timer, &wake) == pdFAIL)
		return -1;
	*_wake = wake;
	return 0;
}
