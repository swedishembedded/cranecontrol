...
int flash_write(uint32_t base, const void *data, size_t size)
{
	...
	for (int c = 0; c < (int)(size >> 2); c++) {
		...
		if (FLASH_ProgramWord(((uintptr_t)&__config_start) + base + (uint32_t)(c << 2),
				      value) != FLASH_COMPLETE) {
			FLASH_Lock();
			return c << 2;
		}
	}
...
static int32_t _flash_sector_from_addr(uint32_t addr)
{
	...
	struct sector {
		uint16_t id;
		uint32_t start, end;
	} sectors[] = { { FLASH_Sector_0, 0x08000000, 0x080003FFF },
...
static int _flash_erase_page_addr(uint32_t addr)
{
	...
	return FLASH_EraseSector((uint16_t)id,
				 VoltageRange_3); // TODO: we assume 3.3v powering scheme
#else
...
{
	if ((base % FLASH_PAGE_SIZE != 0) || base >= CONFIG_FLASH_SIZE)
		return -EINVAL;
...
int flash_read(uint32_t base, void *data, size_t size)
{
	...
	if (base >= (uintptr_t)CONFIG_FLASH_SIZE)
		return -EINVAL;
...
static const struct eeprom_ops _eeprom_ops = { .read = _eeprom_read,
					       .write = _eeprom_write,
