...
USBLIB_EPData EpData[EPCOUNT] = { { 0, EP_CONTROL, 8, 8, 0, 0, 0, 0, 64, 0 },
				  { 1, EP_INTERRUPT, 16, 16, 0, 0, 0, 0, 64, 0 },
				  { 2, EP_BULK, 64, 64, 0, 0, 0, 0, 64, 0 }, //IN  (Device -> Host)
				  { 3, EP_BULK, 64, 64, 0, 0, 0, 0, 64,
				    0 } }; //OUT (Host   -> Device)
...
void USBLIB_Reset(void)
{
	...
	for (uint8_t i = 0; i < EPCOUNT; i++) {
		...
		if (EpData[i].RX_Max >= 64)
			EPBufTable[i].RX_Count.Value =
				0x8000 | (uint16_t)((EpData[i].RX_Max / 64) << 10);
		else
...
void USBLIB_Pma2EPBuf2(uint8_t EPn)
{
	...
	uint32_t *Address =
		(uint32_t *)(USB_PBUFFER + (uint32_t)(EPBufTable[EPn].RX_Address.Value * 2));
...
void USBLIB_GetDescriptor(volatile USBLIB_SetupPacket *SPacket)
{
	...
	case USB_CFG_DESC_TYPE:
		USBLIB_SendData(0, (uint16_t *)&USBD_CDC_CFG_DESCRIPTOR[0],
				sizeof(USBD_CDC_CFG_DESCRIPTOR));
		break;
		...
		if (EPn == 0) { //If control endpoint
			if (EP & USB_EP0R_SETUP) {
				...
				case USB_DEVICE_CDC_REQUEST_GET_LINE_CODING: //0x21
					USBLIB_SendData(EPn, (uint16_t *)&lineCoding,
							sizeof(lineCoding));
					break;
			...
			// Call user function
			uUSBLIB_DataReceivedHandler(EpData[EPn].pRX_BUFF,
						    (uint16_t)EpData[EPn].lRX);
		}
		USB->EPR[EPn] &= 0x78f;
