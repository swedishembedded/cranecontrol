...
static const uint8_t _default_config_desc[] = {
	...
	0x00, 0x04, /* bNumInterfaces: 2 interface */
...
#define foreach_config_desc(d, cd)                                                                 \
	for (struct usb_descriptor_header *d = (struct usb_descriptor_header *)cd;                 \
	     ((uint16_t)((char *)d - (char *)cd)) <                                                \
	     USB_WORD_HL(cd->wTotalLengthH, cd->wTotalLengthL);                                    \
...
static void _alloc_endpoints(struct usbd_device *self)
{
	...
	foreach_config_desc(d, self->config_desc)
	{
		if (d->bDescriptorType == USB_DESC_TYPE_EP) {
			...
			if (idx >= self->endpoint_count || self->endpoints[idx])
				continue;

			struct usb_endpoint *eb =
				(struct usb_endpoint *)kzmalloc(sizeof(struct usb_endpoint));
			usb_endpoint_init(eb, epd);
			self->endpoints[idx] = eb;

			dbg_printk("USB: new %d, %d, %d\n", idx,
				   (((uint8_t *)d - (uint8_t *)self->config_desc)), d->bLength);
		} else {
			dbg_printk("USB: sk %d\n", d->bLength);
		}
	}
...
void usbd_device_init(struct usbd_device *self, void *fdt, int fdt_node,
		      const struct usbd_device_ops *ops)
{
	...
	const struct usb_string_descriptor **sd =
		kzmalloc(sizeof(struct usb_string_descriptor *) * self->string_desc_count);
...
#define _max_len (3 * sizeof(uint32_t) * 2 + 3)
	...
	snprintf(str_serial, _max_len, "%04x-%04x-%04x", (unsigned int)uuid[0],
		 (unsigned int)uuid[1], (unsigned int)uuid[2]);
#undef _max_len
