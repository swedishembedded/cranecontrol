...
static void _uart1_init(void)
{
	...
	// uart itself
	uart.USART_BaudRate =
		9600; // the baudrate is set to the value we passed into this init function
	uart.USART_WordLength =
		USART_WordLength_8b; // we want the data frame size to be 8 bits (standard)
	...
	uart.USART_Mode =
		USART_Mode_Tx | USART_Mode_Rx; // we want to enable the transmitter and the receiver
	USART_Init(
		USART1,
		&uart); // again all the properties are passed to the USART_Init function which takes care of all the bit setting
	...
	nvic.NVIC_IRQChannelPreemptionPriority =
		2; // this sets the priority group of the USART1 interrupts
	...
	NVIC_Init(
		&nvic); // the properties are passed to the NVIC_Init function which takes care of the low level stuff
...
static void _uart3_init(void)
{
	...
	USART_StructInit(&uart);
	uart.USART_BaudRate =
		115200; // the baudrate is set to the value we passed into this init function
	USART_Init(
		USART3,
		&uart); // again all the properties are passed to the USART_Init function which takes care of all the bit setting
...
int stm32_uart_configure(struct stm32_uart *self, uint32_t baud, uint8_t data_bits, uint8_t parity)
{
	...
	// uart itself
	uart.USART_BaudRate =
		baud; // the baudrate is set to the value we passed into this init function
	uart.USART_WordLength =
		(data_bits == 9) ?
			USART_WordLength_9b :
			USART_WordLength_8b; // we want the data frame size to be 8 bits (standard)
...
static const struct serial_ops _serial_ops = { .read = _serial_read, .write = _serial_write };
