...
int _stm32_gpio_register_irq(gpio_pin_t pin, irq_trigger_mode_t mode, struct irq *irq,
			     irq_result_t (*handler)(struct irq *self, int *woken))
{
	struct stm32_gpio_pin *self = container_of(pin, struct stm32_gpio_pin, ops);
	...
	if (pin_idx == 0xff)
		return -EINVAL; // pin not specified
...
static const struct gpio_pin_ops gpio_pin_ops = { .set = &_stm32_gpio_set,
						  .get = &_stm32_gpio_get,
