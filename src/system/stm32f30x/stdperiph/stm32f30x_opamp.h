...
#define IS_OPAMP_ALL_PERIPH(PERIPH)                                                                \
	(((PERIPH) == OPAMP_Selection_OPAMP1) || ((PERIPH) == OPAMP_Selection_OPAMP2) ||           \
	 ((PERIPH) == OPAMP_Selection_OPAMP3) || ((PERIPH) == OPAMP_Selection_OPAMP4))
...
#define OPAMP_InvertingInput_IO1                                                                   \
	((uint32_t)0x00000000) /*!< IO1 (PC5 for OPAMP1 and OPAMP2, PB10 for OPAMP3 and OPAMP4)
                                                                     connected to OPAMPx inverting input */
#define OPAMP_InvertingInput_IO2                                                                   \
	OPAMP_CSR_VMSEL_0 /*!< IO2 (PA3 for OPAMP1, PA5 for OPAMP2, PB2 for OPAMP3, PD8 for OPAMP4)
                                                                      connected to OPAMPx inverting input */
#define OPAMP_InvertingInput_PGA                                                                   \
	OPAMP_CSR_VMSEL_1 /*!< Resistor feedback output connected to OPAMPx inverting input (PGA mode) */
#define OPAMP_InvertingInput_Vout                                                                  \
	OPAMP_CSR_VMSEL /*!< Vout connected to OPAMPx inverting input (follower mode) */

#define IS_OPAMP_INVERTING_INPUT(INPUT)                                                            \
	(((INPUT) == OPAMP_InvertingInput_IO1) || ((INPUT) == OPAMP_InvertingInput_IO2) ||         \
	 ((INPUT) == OPAMP_InvertingInput_PGA) || ((INPUT) == OPAMP_InvertingInput_Vout))
/**
  * @}
  */
...
#define OPAMP_NonInvertingInput_IO1                                                                \
	((uint32_t)0x00000000) /*!< IO1 (PA7 for OPAMP1, PD14 for OPAMP2, PB13 for OPAMP3, PD11 for OPAMP4)
                                                                        connected to OPAMPx non inverting input */
#define OPAMP_NonInvertingInput_IO2                                                                \
	OPAMP_CSR_VPSEL_0 /*!< IO2 (PA5 for OPAMP1, PB14 for OPAMP2, PA5 for OPAMP3, PB11 for OPAMP4)
                                                                         connected to OPAMPx non inverting input */
#define OPAMP_NonInvertingInput_IO3                                                                \
	OPAMP_CSR_VPSEL_1 /*!< IO3 (PA3 for OPAMP1, PB0 for OPAMP2, PA1 for OPAMP3, PA4 for OPAMP4)
                                                                         connected to OPAMPx non inverting input */
#define OPAMP_NonInvertingInput_IO4                                                                \
	OPAMP_CSR_VPSEL /*!< IO4 (PA1 for OPAMP1, PA7 for OPAMP2, PB0 for OPAMP3, PB13 for OPAMP4)
                                                                         connected to OPAMPx non inverting input */

#define IS_OPAMP_NONINVERTING_INPUT(INPUT)                                                         \
	(((INPUT) == OPAMP_NonInvertingInput_IO1) || ((INPUT) == OPAMP_NonInvertingInput_IO2) ||   \
	 ((INPUT) == OPAMP_NonInvertingInput_IO3) || ((INPUT) == OPAMP_NonInvertingInput_IO4))
/**
  * @}
  */
...
#define IS_OPAMP_PGAGAIN(GAIN)                                                                     \
	(((GAIN) == OPAMP_OPAMP_PGAGain_2) || ((GAIN) == OPAMP_OPAMP_PGAGain_4) ||                 \
	 ((GAIN) == OPAMP_OPAMP_PGAGain_8) || ((GAIN) == OPAMP_OPAMP_PGAGain_16))
/**
  * @}
  */
...
#define IS_OPAMP_PGACONNECT(CONNECT)                                                               \
	(((CONNECT) == OPAMP_PGAConnect_No) || ((CONNECT) == OPAMP_PGAConnect_IO1) ||              \
...
#define IS_OPAMP_SECONDARY_INVINPUT(INVINPUT)                                                      \
	(((INVINPUT) == OPAMP_InvertingInput_IO1) || ((INVINPUT) == OPAMP_InvertingInput_IO2))
/**
  * @}
  */
