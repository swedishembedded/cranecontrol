...
#define RTC_FLAGS_MASK                                                                             \
	((uint32_t)(RTC_FLAG_TSOVF | RTC_FLAG_TSF | RTC_FLAG_WUTF | RTC_FLAG_ALRBF |               \
		    ...
		    RTC_FLAG_TAMP2F | RTC_FLAG_TAMP3F | RTC_FLAG_RECALPF | RTC_FLAG_SHPF))
...
ErrorStatus RTC_SetDate(uint32_t RTC_Format, RTC_DateTypeDef *RTC_DateStruct)
{
	...
	if ((RTC_Format == RTC_Format_BIN) && ((RTC_DateStruct->RTC_Month & 0x10) == 0x10)) {
		RTC_DateStruct->RTC_Month =
			(uint8_t)((RTC_DateStruct->RTC_Month & (uint32_t) ~(0x10)) + (uint8_t)0x0A);
	}
	if (RTC_Format == RTC_Format_BIN) {
...
void RTC_ClearFlag(uint32_t RTC_FLAG)
{
	...
	RTC->ISR = (uint32_t)((uint32_t)(~((RTC_FLAG | RTC_ISR_INIT) & 0x0001FFFF) |
...
ITStatus RTC_GetITStatus(uint32_t RTC_IT)
{
	...
	enablestatus =
		(uint32_t)((RTC->CR & RTC_IT) | (tmpreg & ((RTC_IT >> (RTC_IT >> 18)) >> 15)));
