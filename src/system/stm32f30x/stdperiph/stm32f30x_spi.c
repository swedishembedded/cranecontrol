...
void SPI_Init(SPI_TypeDef *SPIx, SPI_InitTypeDef *SPI_InitStruct)
{
	...
	if (SPI_InitStruct->SPI_Mode == SPI_Mode_Master) {
       ...
       master/slave mode, CPOL and CPHA */
		...
		tmpreg |=
			(uint16_t)((uint16_t)(SPI_InitStruct->SPI_Direction |
					      SPI_InitStruct->SPI_Mode) |
				   (uint16_t)((uint16_t)(SPI_InitStruct->SPI_CPOL |
							 SPI_InitStruct->SPI_CPHA) |
					      (uint16_t)((uint16_t)(SPI_InitStruct->SPI_NSS |
								    SPI_InitStruct
...
void I2S_Init(SPI_TypeDef *SPIx, I2S_InitTypeDef *I2S_InitStruct)
{
	...
	/* If the requested audio frequency is not the default, compute the prescaler */
		...
		if (I2S_InitStruct->I2S_MCLKOutput == I2S_MCLKOutput_Enable) {
			...
			tmp = (uint16_t)(((((sourceclock / (32 * (uint32_t)packetlength)) * 10) /
					   I2S_InitStruct->I2S_AudioFreq)) +
					 5);
		}
	...
	tmpreg |= (uint16_t)((uint16_t)(SPI_I2SCFGR_I2SMOD | I2S_InitStruct->I2S_Mode) |
			     (uint16_t)((uint16_t)((uint16_t)(I2S_InitStruct->I2S_Standard |
