...
void _linux_display_task(void *data)
{
	...
	window = SDL_CreateWindow("Simulator", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
	...
	while (1) {
		...
		for (int x = 0; x < self->width; x++) {
			for (int y = 0; y < self->height; y++) {
				...
				SDL_SetRenderDrawColor(self->renderer, (r) ? 0xff : 0,
						       (g) ? 0xff : 0, 0x0, 128);
...
static const struct display_device_ops _display_ops = { .write_pixel = _linux_display_write_pixel };
