...
int linux_uart_write(struct linux_uart *self, const void *_frame, size_t size, uint32_t tout)
{
	...
	if (!(pfd.revents & POLLHUP)) {
		...
		while (written < (ssize_t)size) {
			ssize_t ret = write(self->fd_write, frame + written,
...
static int _pipe_write(serial_port_t serial, const void *_frame, size_t size, uint32_t tout)
{
	struct linux_uart *self = container_of(serial, struct linux_uart, dev.ops);
	return linux_uart_write(self, _frame, size, tout);
}

static int _pipe_read(serial_port_t serial, void *frame, size_t max_size, uint32_t tout_ms)
{
	struct linux_uart *self = container_of(serial, struct linux_uart, dev.ops);
	return linux_uart_read(self, frame, max_size, tout_ms);
}

static const struct serial_device_ops _linux_serial = { .write = _pipe_write, .read = _pipe_read };
