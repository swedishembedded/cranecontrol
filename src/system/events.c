...
static const struct events_device_ops _events_ops = { .subscribe = _events_subscribe,
						      .publish = _events_publish,
						      .publish_from_isr =
							      _events_publish_from_isr };
...
static int _events_probe(void *fdt, int fdt_node)
{
	...
	thread_create(_events_dispatcher, "ev_disp", stack_size, self, prio, NULL);
