...
struct vardir_entry *vardir_find_entry_by_name(struct vardir *self, const char *var_name)
{
	...
	{
		if (entry->name && strncasecmp(var_name, entry->name, strlen(entry->name)) == 0 &&
		    strlen(var_name) == strlen(entry->name)) {
			return entry;
		}
	}
	return NULL;
...
{
	if (!self || !_var_value_valid(self, (float)value))
		return -1;
...
{
	uint32_t v;
	if (vardir_entry_get_u32(vardir_find_entry_by_id(self, id), &v) < 0)
		return -1;
	*value = (int32_t)v;
	return 0;
}
		     ...
		     uint32_t default_value)
{
	...
	if (!entry)
		return -ENOMEM;
	const struct vardir_entry e = {
		.id = id, .name = name, .type = type, .value = { .u32 = default_value }
	};
	memcpy(entry, &e, sizeof(struct vardir_entry));
	list_add(&entry->list, &self->directory);
	return 0;
		     ...
		     struct vardir_entry_ops **ops)
{
	...
	if (!entry)
		return -ENOMEM;
	const struct vardir_entry e = { .id = id,
					.name = name,
