						...
						uint32_t delta)
{
	...
	fdt_for_each_property_offset(fixup_prop, fdto, fixup_node)
	{
		...
		if (fixup_len % (int)sizeof(uint32_t))
		...
		for (i = 0; i < (fixup_len / (int)sizeof(uint32_t)); i++) {
			fdt32_t adj_val;
			uint32_t poffset;
			...
			ret = fdt_setprop_inplace_namelen_partial(fdto, tree_node, name,
								  (int)strlen(name), poffset,
								  &adj_val, sizeof(adj_val));
				     ...
				     int poffset, const char *label)
{
	...
	fixup_off = (int)fdt_path_offset_namelen(fdto, path, (int)path_len);
	...
	return fdt_setprop_inplace_namelen_partial(fdto, fixup_off, name, (int)name_len,
						   (uint32_t)poffset, &phandle_prop, sz);
}
...
static int overlay_fixup_phandle(void *fdt, void *fdto, int symbols_off, int property)
{
	...
	do {
		...
		path = fixup_str;
		sep = memchr(fixup_str, ':', (size_t)fixup_len);
		if (!sep || *sep != ':')
			return -FDT_ERR_BADOVERLAY;
		...
		name = sep + 1;
		sep = memchr(name, ':', (size_t)fixup_len);
		if (!sep || *sep != ':')
			return -FDT_ERR_BADOVERLAY;
		...
		poffset = (int)strtoul(sep + 1, &endptr, 10);
		if ((*endptr != '\0') || (endptr <= (sep + 1)))
			return -FDT_ERR_BADOVERLAY;

		ret = overlay_fixup_one_phandle(fdt, fdto, symbols_off, path, (uint32_t)path_len,
						name, (uint32_t)name_len, poffset, label);
		if (ret)
			return ret;
	} while (len > 0);
...
static int overlay_symbol_update(void *fdt, void *fdto)
{
	...
	fdt_for_each_property_offset(prop, fdto, ov_sym)
	{
		...
		if (path_len < 1 || memchr(path, '\0', (size_t)path_len) != &path[path_len - 1])
		...
		if ((e - s) < len || memcmp(s, "/__overlay__/", (size_t)len))
			return -FDT_ERR_BADOVERLAY;
		...
		if (len > 1) { /* target is not root */
			if (!target_path) {
				...
				memcpy(buf, target_path, (size_t)(len + 1));

		} else
			len--;

		buf[len] = '/';
		memcpy(buf + len + 1, rel_path, (size_t)rel_path_len);
