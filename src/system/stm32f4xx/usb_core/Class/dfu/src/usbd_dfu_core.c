...
static uint8_t usbd_dfu_Setup(void *pdev, USB_SETUP_REQ *req)
{
	...
	case USB_REQ_TYPE_STANDARD:
		...
		case USB_REQ_GET_INTERFACE:
			USBD_CtlSendData(pdev, (uint8_t *)&usbd_dfu_AltSet, 1);
			break;
...
static void DFU_Req_DNLOAD(void *pdev, USB_SETUP_REQ *req)
{
	...
	if (req->wLength > 0) {
		if ((DeviceState == STATE_dfuIDLE) || (DeviceState == STATE_dfuDNLOAD_IDLE)) {
			...
			USBD_CtlPrepareRx(pdev, (uint8_t *)MAL_Buffer, wlength);
		}
		/* Unsupported state */
		else {
			/* Call the error management function (command will be nacked */
	...
	if (req->wLength > 0) {
		if ((DeviceState == STATE_dfuIDLE) || (DeviceState == STATE_dfuUPLOAD_IDLE)) {
			...
			if (wBlockNum == 0) {
				...
				USBD_CtlSendData(pdev, (uint8_t *)(&(MAL_Buffer[0])), 3);
			} else if (wBlockNum > 1) {
				DeviceState = STATE_dfuUPLOAD_IDLE;
				DeviceStatus[4] = DeviceState;
				DeviceStatus[1] = 0;
				DeviceStatus[2] = 0;
				DeviceStatus[3] = 0;
				Addr = ((wBlockNum - 2) * XFERSIZE) +
				       Pointer; /* Change is Accelerated*/

				/* Return the physical address where data are stored */
				Phy_Addr = MAL_Read(Addr, wlength);

				/* Send the status data over EP0 */
				USBD_CtlSendData(pdev, Phy_Addr, wlength);
...
static void DFU_Req_GETSTATUS(void *pdev)
{
	...
	/* Send the status data over EP0 */
	USBD_CtlSendData(pdev, (uint8_t *)(&(DeviceStatus[0])), 6);
}
...
static void DFU_Req_GETSTATE(void *pdev)
{
	...
	USBD_CtlSendData(pdev, &DeviceState, 1);
}
...
static uint8_t *USBD_DFU_GetUsrStringDesc(uint8_t speed, uint8_t index, uint16_t *length)
{
	...
	if (index <= (USBD_IDX_INTERFACE_STR + USBD_ITF_MAX_NUM)) {
		USBD_GetString((uint8_t *)usbd_dfu_StringDesc[index - USBD_IDX_INTERFACE_STR - 1],
			       USBD_StrDesc, length);
		return USBD_StrDesc;
	}
