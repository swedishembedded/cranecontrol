...
static int8_t SCSI_Write10(uint8_t lun, uint8_t *params)
{
	if (MSC_BOT_State == BOT_IDLE) /* Idle */
	{
		...
		DCD_EP_PrepareRx(cdev, MSC_OUT_EP, MSC_BOT_Data,
...
static int8_t SCSI_ProcessRead(uint8_t lun)
{
	...
	if (USBD_STORAGE_fops->Read(lun, MSC_BOT_Data, SCSI_blk_addr / SCSI_blk_size,
				    len / SCSI_blk_size) < 0) {
...
static int8_t SCSI_ProcessWrite(uint8_t lun)
{
	...
	if (USBD_STORAGE_fops->Write(lun, MSC_BOT_Data, SCSI_blk_addr / SCSI_blk_size,
				     len / SCSI_blk_size) < 0) {
