...
__ALIGN_BEGIN uint8_t USBD_MSC_CfgDesc[USB_MSC_CONFIG_DESC_SIZ] __ALIGN_END = {
	...
	LOBYTE(MSC_MAX_PACKET), HIBYTE(MSC_MAX_PACKET), 0x00, /*Polling interval in milliseconds */
...
__ALIGN_BEGIN uint8_t USBD_MSC_OtherCfgDesc[USB_MSC_CONFIG_DESC_SIZ] __ALIGN_END = {
	...
	0x09, /* bLength: Configuation Descriptor size */
	USB_DESC_TYPE_OTHER_SPEED_CONFIGURATION, USB_MSC_CONFIG_DESC_SIZ,
	...
	case USB_REQ_TYPE_STANDARD:
		...
		case USB_REQ_CLEAR_FEATURE:
			...
			if ((((uint8_t)req->wIndex) & 0x80) == 0x80) {
				DCD_EP_Open(pdev, ((uint8_t)req->wIndex), MSC_EPIN_SIZE,
					    USB_OTG_EP_BULK);
			} else {
				DCD_EP_Open(pdev, ((uint8_t)req->wIndex), MSC_EPOUT_SIZE,
					    USB_OTG_EP_BULK);
			}
