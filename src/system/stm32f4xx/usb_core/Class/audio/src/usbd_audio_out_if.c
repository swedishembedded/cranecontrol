...
/** @defgroup usbd_audio_out_if_Private_Variables
  * @{
  */
AUDIO_FOPS_TypeDef AUDIO_OUT_fops = { Init,    DeInit,	   AudioCmd, VolumeCtl,
				      MuteCtl, PeriodicTC, GetState };
...
static uint8_t AudioCmd(uint8_t *pbuf, uint32_t size, uint8_t cmd)
{
	...
	case AUDIO_CMD_PLAY:
		...
		if ((AudioState == AUDIO_STATE_ACTIVE) || (AudioState == AUDIO_STATE_STOPPED) ||
