...
USBD_Class_cb_TypeDef AUDIO_cb = {
	usbd_audio_Init,	usbd_audio_DeInit,     usbd_audio_Setup,   NULL, /* EP0_TxSent */
	usbd_audio_EP0_RxReady, usbd_audio_DataIn,     usbd_audio_DataOut, usbd_audio_SOF, NULL,
	usbd_audio_OUT_Incplt,	USBD_audio_GetCfgDesc,
...
static uint8_t usbd_audio_CfgDesc[AUDIO_CONFIG_DESC_SIZE] = {
	...
	HIBYTE(AUDIO_CONFIG_DESC_SIZE), 0x02, /* bNumInterfaces */
	...
	AUDIO_PACKET_SZE(
		USBD_AUDIO_FREQ), /* wMaxPacketSize in Bytes (Freq(Samples)*2(Stereo)*2(HalfWord)) */
	0x01, /* bInterval */
	0x00, /* bRefresh */
	0x00, /* bSynchAddress */
	/* 09 byte*/
...
static uint8_t usbd_audio_EP0_RxReady(void *pdev)
{
	...
	if (AudioCtlCmd ==
	    AUDIO_REQ_SET_CUR) { /* In this driver, to simplify code, only SET_CUR request is managed */
		/* Check for which addressed unit the AudioControl request has been issued */
		if (AudioCtlUnit ==
		    AUDIO_OUT_STREAMING_CTRL) { /* In this driver, to simplify code, only one unit is manage */
...
static uint8_t usbd_audio_DataOut(void *pdev, uint8_t epnum)
{
	if (epnum == AUDIO_OUT_EP) {
		...
		if (IsocOutWrPtr >=
		    (IsocOutBuff +
		     (AUDIO_OUT_PACKET * OUT_PACKET_NUM))) { /* All buffers are full: roll back */
			IsocOutWrPtr = IsocOutBuff;
		} else { /* Increment the buffer pointer */
		...
		if ((PlayFlag == 0) &&
		    (IsocOutWrPtr >= (IsocOutBuff + ((AUDIO_OUT_PACKET * OUT_PACKET_NUM) / 2)))) {
...
static uint8_t usbd_audio_SOF(void *pdev)
{
	...
	if (PlayFlag) {
		...
		if (IsocOutRdPtr >=
		    (IsocOutBuff +
		     (AUDIO_OUT_PACKET * OUT_PACKET_NUM))) { /* Roll back to the start of buffer */
		...
		if (IsocOutRdPtr == IsocOutWrPtr) {
			...
			AUDIO_OUT_fops.AudioCmd(
				(uint8_t *)(IsocOutBuff), /* Samples buffer pointer */
				AUDIO_OUT_PACKET, /* Number of samples in Bytes */
