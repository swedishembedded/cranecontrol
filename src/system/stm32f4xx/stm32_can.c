...
static const struct can_device_ops _stm32_can_ops = { .send = _can_send,
...
static int _memory_read(memory_device_t dev, size_t offset, void *data, size_t size)
{
	...
	if (size != sizeof(struct can_counters) || offset != 0)
		return -EINVAL;
	//thread_mutex_lock(&self->lock);
...
static struct memory_device_ops _mem_ops = { .read = _memory_read, .write = _memory_write };
...
static int _stm32_can_probe(void *fdt, int fdt_node)
{
	...
	can.CAN_Prescaler =
		(uint16_t)(prescaler); //(uint16_t)(clocks.PCLK1_Frequency / (uint32_t)((uint32_t)(can.CAN_BS1 + can.CAN_BS2 + can.CAN_SJW + 1) * (uint32_t)baud));
	...
	CAN_ITConfig(CANx,
		     CAN_IT_FMP0 | CAN_IT_FMP1 | CAN_IT_TME | CAN_IT_ERR | CAN_IT_LEC | CAN_IT_BOF |
			     CAN_IT_EPV | CAN_IT_EWG | CAN_IT_FOV1 | CAN_IT_FOV0,
		     ENABLE);
	...
	if (console) {
		console_add_command(console, self, fdt_get_name(fdt, fdt_node, NULL),
				    "STM32 CAN interface", "", _stm32_can_cmd);
	}
	...
	printk(PRINT_DEFAULT "can: sysclk %d, pclk1 %d, pclk2 %d, hclk %d\n",
	       clocks.SYSCLK_Frequency, clocks.PCLK1_Frequency, clocks.PCLK2_Frequency,
