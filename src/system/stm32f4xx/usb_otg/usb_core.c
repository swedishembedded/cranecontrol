...
* @retval USB_OTG_STS : status
*/
USB_OTG_STS USB_OTG_WritePacket(USB_OTG_CORE_HANDLE *pdev, uint8_t *src, uint8_t ch_ep_num,
				uint16_t len)
{
	USB_OTG_STS status = USB_OTG_OK;
...
USB_OTG_STS USB_OTG_SelectCore(USB_OTG_CORE_HANDLE *pdev, USB_OTG_CORE_ID_TypeDef coreID)
{
	...
	for (i = 0; i < pdev->cfg.dev_endpoints; i++) {
		pdev->regs.INEP_REGS[i] =
			(USB_OTG_INEPREGS *)(baseAddress + USB_OTG_DEV_IN_EP_REG_OFFSET +
					     (i * USB_OTG_EP_REG_OFFSET));
		pdev->regs.OUTEP_REGS[i] =
			(USB_OTG_OUTEPREGS *)(baseAddress + USB_OTG_DEV_OUT_EP_REG_OFFSET +
					      (i * USB_OTG_EP_REG_OFFSET));
	}
	...
	for (i = 0; i < pdev->cfg.host_channels; i++) {
		pdev->regs.HC_REGS[i] =
			(USB_OTG_HC_REGS *)(baseAddress + USB_OTG_HOST_CHAN_REGS_OFFSET +
...
USB_OTG_STS USB_OTG_HC_StartXfer(USB_OTG_CORE_HANDLE *pdev, uint8_t hc_num)
{
	...
	if (pdev->host.hc[hc_num].xfer_len > 0) {
		num_packets =
			(pdev->host.hc[hc_num].xfer_len + pdev->host.hc[hc_num].max_packet - 1) /
			pdev->host.hc[hc_num].max_packet;

		if (num_packets > max_hc_pkt_count) {
			num_packets = max_hc_pkt_count;
			pdev->host.hc[hc_num].xfer_len =
				num_packets * pdev->host.hc[hc_num].max_packet;
		}
	} else {
		num_packets = 1;
	}
...
USB_OTG_STS USB_OTG_CoreInitDev(USB_OTG_CORE_HANDLE *pdev)
{
	...
	if (pdev->cfg.coreID == USB_OTG_FS_CORE_ID) {
		...
		txfifosize.b.startaddr =
			(uint16_t)(nptxfifosize.b.startaddr + nptxfifosize.b.depth);
...
USB_OTG_STS USB_OTG_EPStartXfer(USB_OTG_CORE_HANDLE *pdev, USB_OTG_EP *ep)
{
	...
	if (ep->is_in == 1) {
      ...
      */
			...
			deptsiz.b.pktcnt =
				((ep->xfer_len - 1 + ep->maxpacket) / ep->maxpacket) & 0x3ff;
		...
		if (pdev->cfg.dma_enable == 1) {
			...
			if (ep->type != EP_TYPE_ISOC) {
				...
				if (ep->xfer_len > 0) {
					...
					USB_OTG_MODIFY_REG32(&pdev->regs.DREGS->DIEPEMPMSK, 0,
							     fifoemptymsk);
				}
			}
		}
    ...
    */
		if (ep->xfer_len == 0) {
			...
			deptsiz.b.pktcnt =
				((ep->xfer_len + (ep->maxpacket - 1)) / ep->maxpacket) & 0x3ff;
	...
	if (ep->is_in == 1) {
		...
		if (pdev->cfg.dma_enable == 0) {
			...
			if (ep->xfer_len > 0) {
				{
					fifoemptymsk =
						(uint32_t)(fifoemptymsk | (uint32_t)(1 << ep->num));
					USB_OTG_MODIFY_REG32(&pdev->regs.DREGS->DIEPEMPMSK, 0,
							     fifoemptymsk);
				}
			}
