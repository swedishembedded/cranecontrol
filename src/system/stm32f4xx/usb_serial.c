...
static int _stm32_usb_cmd(console_device_t con, void *ptr, int argc, char **argv)
{
	if (argc == 2 && strcmp(argv[1], "status") == 0) {
		static const struct {
			int bit;
			const char *name;
		} bits[] = { { .bit = 0, "SRQSCS" },   { .bit = 1, "SRQ" },
			     { .bit = 8, "HNGSCS" },   { .bit = 9, "HNPRQ" },
			     { .bit = 10, "HSHNPEN" }, { .bit = 11, "DHNPEN" },
			     { .bit = 16, "CIDSTS" },  { .bit = 17, "DBCT" },
			     { .bit = 18, "ASVLD" },   { .bit = 19, "BSVLD" } };
...
static int _usb_serial_read(serial_port_t dev, void *buf, size_t size, uint32_t timeout)
{
...
static int _usb_serial_write(serial_port_t dev, const void *buf, size_t size, uint32_t timeout)
{
	uint32_t cnt = 0;
	while (cnt < size) {
