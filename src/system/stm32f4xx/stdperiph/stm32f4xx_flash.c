	...
	if (status == FLASH_COMPLETE) {
		if (NewState != DISABLE) {
			...
			*(__IO uint16_t *)OPTCR_BYTE2_ADDRESS =
				(uint16_t)(*(__IO uint16_t *)OPTCR_BYTE2_ADDRESS | OB_WRP);
		}
	}
}
	...
	if (status == FLASH_COMPLETE) {
		if (NewState != DISABLE) {
			...
			*(__IO uint16_t *)OPTCR1_BYTE2_ADDRESS =
				(uint16_t)(*(__IO uint16_t *)OPTCR1_BYTE2_ADDRESS | OB_WRP);
		}
	}
}
...
void FLASH_OB_PCROPConfig(uint32_t OB_PCROP, FunctionalState NewState)
{
	...
	if (status == FLASH_COMPLETE) {
		if (NewState != DISABLE) {
			*(__IO uint16_t *)OPTCR_BYTE2_ADDRESS =
				(uint16_t)(*(__IO uint16_t *)OPTCR_BYTE2_ADDRESS | OB_PCROP);
		} else {
...
void FLASH_OB_PCROP1Config(uint32_t OB_PCROP, FunctionalState NewState)
{
	...
	if (status == FLASH_COMPLETE) {
		if (NewState != DISABLE) {
			*(__IO uint16_t *)OPTCR1_BYTE2_ADDRESS =
				(uint16_t)(*(__IO uint16_t *)OPTCR1_BYTE2_ADDRESS | OB_PCROP);
		} else {
