...
static int _stm32_cpu_probe(void *fdt, int fdt_node)
{
	...
	if (ready) {
		...
		hclk_div = (hclk_div == 1) ?
				   0 :
				   ((uint32_t)(0x80 | (((uint16_t)ffs((int)hclk_div) - 2) << 4)));
		pclk1_div =
			(pclk1_div == 1) ?
				0 :
				((uint32_t)(0x1000 | (((uint16_t)ffs((int)pclk1_div) - 2) << 10)));
		pclk2_div =
			(pclk2_div == 1) ?
				0 :
				((uint32_t)(0x8000 | (((uint16_t)ffs((int)pclk2_div) - 2) << 13)));

		RCC->CFGR |= hclk_div | pclk1_div | pclk2_div;
		RCC->PLLCFGR = pllm | (plln << 6) | (((pllp >> 1) - 1) << 16) |
			       RCC_PLLCFGR_PLLSRC_HSE | (pllq << 24);
...
static int _stm32_cmd_cpuinfo(console_device_t con, void *userptr, int argc, char **argv)
{
	...
	console_printf(con,
		       "RCC: pllm: %d, plln: %d, pllp: %d, pllsrc = %d, pllq = %d, pllr = %d\n",
	...
	console_printf(con, "Processor clock speed: %d (%s)\n", time_get_clock_speed(),
		       (time_cpu_clock_speed_exact()) ? "exact" : "inexact");
	console_printf(con, "SYSCLK: %d, HCLK: %d, PCLK1: %d, PCLK2: %d, ", clocks.SYSCLK_Frequency,
		       clocks.HCLK_Frequency, clocks.PCLK1_Frequency, clocks.PCLK2_Frequency);
