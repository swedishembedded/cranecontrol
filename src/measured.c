...
void _fb_update_measurements(struct fb *self) {
	...
	self->measured.vmot =
	    _scale_input((float)self->inputs.vmot, &self->config.limit.vmot);
	...
	self->measured.ia_pitch =
	    ((float)self->inputs.ia_pitch - (float)self->config.dc_cal.pitch_a) / scale /
	    gp / res;
	self->measured.ib_pitch =
	    ((float)self->inputs.ib_pitch - (float)self->config.dc_cal.pitch_b) / scale /
	    gp / res;
	self->measured.ia_yaw =
	    ((float)self->inputs.ia_yaw - (float)self->config.dc_cal.yaw_a) / scale / gy /
	    res;
	self->measured.ib_yaw =
	    ((float)self->inputs.ib_yaw - (float)self->config.dc_cal.yaw_b) / scale / gy /
	    res;
