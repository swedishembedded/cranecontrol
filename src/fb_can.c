...
static int _fb_can_reinit_slaves(struct fb *self)
{
	...
	conf = (struct canopen_pdo_config){
		...
		.map = { CANOPEN_PDO_MAP_ENTRY(CANOPEN_FB_PITCH_POSITION, CANOPEN_PDO_SIZE_16),
			 ...
			 CANOPEN_PDO_MAP_ENTRY(CANOPEN_FB_YAW_CURRENT, CANOPEN_PDO_SIZE_16), 0 }
	};
...
static ssize_t _mfr_range_read(regmap_range_t range, uint32_t addr, regmap_value_type_t type,
			       void *data, size_t size)
{
	...
	case CANOPEN_FB_PITCH_CURRENT: {
		ret = regmap_convert_u16((uint16_t)(int16_t)(self->measured.ia_pitch * 1000), type,
					 data, size);
	} break;
	case CANOPEN_FB_YAW_CURRENT: {
		ret = regmap_convert_u16((uint16_t)(int16_t)(self->measured.ia_yaw * 1000), type,
					 data, size);
	} break;
	case CANOPEN_FB_PITCH_POSITION: {
		ret = regmap_convert_u16((uint16_t)(int16_t)(self->measured.pitch * 1000), type,
					 data, size);
	} break;
	case CANOPEN_FB_YAW_POSITION: {
		ret = regmap_convert_u16((uint16_t)(int16_t)(self->measured.yaw * 1000), type, data,
					 size);
	} break;
	...
	case CANOPEN_FB_VMOT: {
		uint16_t vmot = (uint16_t)constrain_i32((int32_t)(self->measured.vmot * 1000), 0,
							UINT16_MAX);
...
static ssize_t _mfr_range_write(regmap_range_t range, uint32_t addr, regmap_value_type_t type,
				const void *data, size_t size)
{
	struct fb *self = container_of(range, struct fb, mfr_range.ops);
...
static bool _fb_slave_timed_out(struct fb *self)
{
	...
	uint32_t mask = FB_SLAVE_VALID_PITCH | FB_SLAVE_VALID_YAW | FB_SLAVE_VALID_I_PITCH |
...
void _fb_can_monitor_link_state(struct fb *self)
{
	if (self->mode == FB_MODE_SLAVE) {
		if (timestamp_expired(self->remote.pitch_update_timeout) ||
			...
			self->remote.pitch_update_timeout =
				timestamp_from_now_us(FB_REMOTE_TIMEOUT);
