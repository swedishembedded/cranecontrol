...
static void _hslider(console_device_t con, int px, int py, int val, int val_min, int val_max,
		     int size)
{
	GOTO(px, py);
...
static void _ui_update(struct fb *self)
{
	...
	static const char *control_modes[] = { "   OFF   ", " REMOTE  ", " MANUAL  ", "  AUTO   " };

	HOME();
	NAME("Board Mode");
	BOOL(self->mode == FB_MODE_MASTER, "MASTER", "SLAVE");
	NL();
	NAME("Control Mode");
	CHOICE(self->control_mode, control_modes);
	NL();

	console_printf(con, "%-17s%-10s%-11s%-11s\n", "", "Phase A", "Phase B", "Phase C");
	NAME("VS PITCH (mV)");
	FLOAT(meas.vsa_pitch);
	FLOAT(meas.vsb_pitch);
	FLOAT(meas.vsb_pitch);
	NL();
	NAME("VS YAW (mV)");
	FLOAT(meas.vsa_yaw);
	...
	NAME("Joystick");
	FLOAT(meas.joy_pitch);
	FLOAT(meas.joy_yaw);
	NL();
	NAME("Intensity");
	FLOAT(meas.pitch_acc);
	FLOAT(meas.yaw_acc);
	NL();
	NAME("Speed");
	FLOAT(meas.pitch_speed);
	FLOAT(meas.yaw_speed);
	NL();
	NAME("Position");
	FLOAT(meas.pitch);
	FLOAT(meas.yaw);
