...
void motion_profile_plan_move(struct motion_profile *self, const struct timeval *current_time, float dstart, float vstart, float dend, float vend){
	...
	*/
	...
	self->t1 = (-vend * (acc + dec) + sqrtf((acc + dec) * (2 * acc * dec * dist + acc * vend * vend + dec * vstart * vstart)) + (acc + dec) * (vend - vstart)) / (acc * (acc + dec));
	self->t2 = 0;
	self->t3 = (-vend * (acc + dec) + sqrtf((acc + dec) * (2 * acc * dec * dist + acc * vend * vend + dec * vstart * vstart))) / (dec * (acc + dec));

	self->v2 = vstart + acc * self->t1;
	if(fabsf(self->v2) > fabsf(vmax)){
		self->t1 = (vmax - vstart) / acc;
		self->t2 = (2 * acc * dec * dist + acc * vend * vend - acc * vmax * vmax - dec * vmax * vmax + dec * vstart * vstart) / (2 * acc * dec * vmax);
	...
	self->start_time.tv_sec = current_time->tv_sec;
	self->start_time.tv_usec = current_time->tv_usec;
}
...
int motion_profile_get_pva(struct motion_profile *self, const struct timeval *current_time, float *pos_sp, float *vel_sp, float *acc_sp){
	if(!self->move_planned) return -1;

	struct timeval diff;
	timeval_subtract(current_time, &self->start_time, &diff);
	float t = (float)diff.tv_sec + ((float)diff.tv_usec * 1e-6);
	float v = self->v3;
	...
	if(t < self->t1){
		...
		a = acc;
		self->state = MOTION_PROFILE_ACCELERATING;
	} else if(t >= self->t1 && t < (self->t1 + self->t2)){
		v = self->v2;
		d = self->d1 + (t - self->t1) * self->v2;
		a = 0;
		self->state = MOTION_PROFILE_CRUISING;
	} else if(t >= (self->t1 + self->t2) && t < self->total_time){
		v = self->v2 - dec * (t - self->t1 - self->t2);
		d = self->total_distance - self->v3 * (self->total_time - t) - (dec * (self->total_time - t) * (self->total_time - t)) / 2;
		a = -dec;
		self->state = MOTION_PROFILE_DECELERATING;
	} else {
		self->state = MOTION_PROFILE_COMPLETED;
	}

	// just a precaution. Not that it will ever happen during normal operation..
	if(isnan(v) || isnan(d)) return -1;
...
bool motion_profile_completed(struct motion_profile *self, const struct timeval *current_time){
	if(!self->move_planned) return true;
	struct timeval diff;
	timeval_subtract(current_time, &self->start_time, &diff);
	float t = (float)diff.tv_sec + ((float)diff.tv_usec * 1e-6);
	if(t > self->total_time) return true;
	return false;
}
