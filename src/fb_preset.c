...
int fb_try_load_preset(struct fb *self, unsigned preset) {
	...
	if(preset == FB_PRESET_HOME) {
		...
		fb_control_set_limits(
		    &self->axis[FB_AXIS_UPDOWN], self->measured.pitch_acc * 0.5,
		    self->measured.pitch_speed * 0.9, self->measured.pitch_acc * 0.5);
		fb_control_set_limits(
		    &self->axis[FB_AXIS_LEFTRIGHT], self->measured.yaw_acc * 0.6,
		...
		float t_leftright =
		    fb_control_get_remaining_time(&self->axis[FB_AXIS_LEFTRIGHT]);
		...
		if(fabsf(t_min) > 0.05f) {
			...
			if(fabsf(scale) > 0.05) {
				if(t_updown >= t_leftright) {
					...
					fb_control_set_timebase(&self->axis[FB_AXIS_LEFTRIGHT], 1.f / scale);
					...
					fb_control_set_timebase(&self->axis[FB_AXIS_UPDOWN], 1.f / scale);
				}
			} else {
