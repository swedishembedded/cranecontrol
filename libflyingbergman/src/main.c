...
static void _fb_enter_state(struct application *self, void (*fp)(struct application *self, float dt)) {
	...
	if(fp == _fb_state_wait_home) {
		...
		for(unsigned c = 0; c < FB_PRESET_COUNT; c++) {
			...
			switch(c) {
			case 0:
				led = &self->state.leds[FB_LED_HOME];
				break;
			case 1:
				led = &self->state.leds[FB_LED_PRESET1];
				break;
			case 2:
				led = &self->state.leds[FB_LED_PRESET2];
				break;
			case 3:
				led = &self->state.leds[FB_LED_PRESET3];
				break;
			case 4:
				led = &self->state.leds[FB_LED_PRESET4];
				break;
			default:
				continue;
			}
...
static void _fb_state_operational(struct application *self, float dt) {
	...
	if(num_pressed == 1) {
		...
		} else if(sw[preset]->pressed && timestamp_expired(ts)) {
...
static void _fb_load_config(struct application *self) {
	...
	self->config.limit.pitch = (struct fb_analog_limit) {
		.min = 0, .max = 3790, .omin = -1.f, .omax = 1.f
	};
	self->config.limit.joy_pitch = (struct fb_analog_limit) {
		.min = 0, .max = 4096, .omin = -1.f, .omax = 1.f
	};
	self->config.limit.joy_yaw = (struct fb_analog_limit) {
		.min = 0, .max = 4096, .omin = -1.f, .omax = 1.f
	};
	self->config.limit.pitch_acc = (struct fb_analog_limit) {
		.min = 0, .max = 4096, .omin = FB_PITCH_MAX_ACC * 0.2, .omax = FB_PITCH_MAX_ACC
	};
	self->config.limit.yaw_acc = (struct fb_analog_limit) {
		.min = 0, .max = 4096, .omin = FB_YAW_MAX_ACC * 0.2, .omax = FB_YAW_MAX_ACC
	};
	self->config.limit.pitch_speed = (struct fb_analog_limit) {
		.min = 0, .max = 4096, .omin = FB_PITCH_MAX_VEL * 0.2, .omax = FB_PITCH_MAX_VEL
	};
	self->config.limit.yaw_speed = (struct fb_analog_limit) {
		.min = 0, .max = 4096, .omin = FB_YAW_MAX_VEL * 0.2, .omax = FB_YAW_MAX_VEL
	};
	self->config.limit.vmot = (struct fb_analog_limit) {
		.min = 0, .max = 3470, .omin = 0, .omax = 80
	};
	self->config.limit.temp_yaw = (struct fb_analog_limit) {
		.min = 860, .max = 2000, .omin = 100, .omax = 100.f
	};
	self->config.limit.temp_pitch = (struct fb_analog_limit) {
